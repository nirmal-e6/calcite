...
@Value.Enclosing
public class E6JoinToMultiJoinWithOrder extends RelRule<E6JoinToMultiJoinWithOrder.Config>
  ...
  // build a MultiJoin whose inputs are exactly 'orderedInputs', aligned with the winner’s leaf
  // order
  static MultiJoin buildMultiJoinInOrder(
      RelOptCluster cluster, List<RelNode> orderedInputs, RelNode rootOfBinaryTree) {
    ...
    boolean topIsOuter =
        (rootOfBinaryTree instanceof Join)
            && ((Join) rootOfBinaryTree).getJoinType() != JoinRelType.INNER;
    ...
    if (topIsOuter) {
      ...
      if (n != 2) {
        throw new IllegalStateException(
            "Top outer-join island must map to exactly 2 inputs; got " + n);
      }

      final RexNode remapped =
          top.getCondition().accept(createRemapperFor(top, localLayouts, startOfFactor));
      switch (joinType) {
      case LEFT:
      ...
      collectInnerConjuncts(
          rootOfBinaryTree, innerConjuncts, singleSideConjuncts, localLayouts, startOfFactor);
    }

    final RexNode joinFilter =
        innerConjuncts.isEmpty()
            ? rb.makeLiteral(true)
            : RexUtil.composeConjunction(rb, innerConjuncts, false);

    final RexNode postJoinFilter =
        singleSideConjuncts.isEmpty()
            ? null
    ...
    com.google.common.collect.ImmutableMap<Integer, ImmutableIntList> refCounts =
        computeRefCounts(n, fieldCountOfFactor, startOfFactor, innerConjuncts, postJoinFilter);
  ...
  // Creates a RexShuttle that remaps field references from a local join's coordinate system to the
  // global MultiJoin's coordinate system.
  private static LocalToGlobalRemapper createRemapperFor(
      RelNode joinNode, HashMap<RelNode, LocalLayout> localLayouts, int[] startOfFactor) {
  ...
  static void buildLocalLayouts(
      RelNode node, HashMap<RelNode, Integer> leafToFactor, HashMap<RelNode, LocalLayout> out) {
  ...
  static void collectInnerConjuncts(
      RelNode node,
      List<RexNode> innerConjuncts,
      List<RexNode> singleSideConjuncts,
      HashMap<RelNode, LocalLayout> localLayouts,
      int[] startOfFactor) {
    if (node instanceof Project || node instanceof Filter) {
    ...
    if (node instanceof Join) {
      ...
      if (j.getJoinType() == JoinRelType.INNER) {
        ...
        collectInnerConjuncts(
            j.getLeft(), innerConjuncts, singleSideConjuncts, localLayouts, startOfFactor);
        collectInnerConjuncts(
            j.getRight(), innerConjuncts, singleSideConjuncts, localLayouts, startOfFactor);
      }
    }
  }
  ...
  static com.google.common.collect.ImmutableMap<Integer, ImmutableIntList> computeRefCounts(
      int n,
      int[] fieldCountOfFactor,
      int[] startOfFactor,
      List<RexNode> innerConjuncts,
  ...
  public interface Config extends RelRule.Config {
    E6JoinToMultiJoinWithOrder.Config DEFAULT =
        ImmutableE6JoinToMultiJoinWithOrder.Config.of().withOperandFor(LogicalJoin.class);
