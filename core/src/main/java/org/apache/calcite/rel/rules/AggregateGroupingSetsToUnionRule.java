...
/**
 * E6 Copy - THIS RULE HAS BEEN COPIED HERE AS IT WAS NOT AVAILABLE IN THE CALCITE VERSION WE HAD.
 * REMOVE THIS ONCE WE UPGRADE TO THE VERSION CONTAINING THIS RULE
 *
 * <p>Rule that converts a {@link Aggregate} with {@code GROUPING SETS} into a {@code UNION ALL} of
 * simpler aggregates.
 *
...
public class AggregateGroupingSetsToUnionRule
    extends RelRule<AggregateGroupingSetsToUnionRule.Config> implements SubstitutionRule {
  ...
  @Override public void onMatch(RelOptRuleCall call) {
    ...
    for (ImmutableBitSet subGroupSet : aggregate.getGroupSets()) {
      ...
      for (AggregateCall aggCall : aggregate.getAggCallList()) {
        ...
        default:
          subProjects.add(
                new RexInputRef(subGroupSet.cardinality() + subAggCalls.size(), aggCall.getType()));
      ...
      relBuilder
          .aggregate(relBuilder.groupKey(subGroupSet), subAggCalls)
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT =
        ImmutableAggregateGroupingSetsToUnionRule.Config.of()
