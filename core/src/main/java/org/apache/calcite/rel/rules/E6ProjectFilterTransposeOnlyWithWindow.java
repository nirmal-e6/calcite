...
@Value.Enclosing
public class E6ProjectFilterTransposeOnlyWithWindow
    extends RelRule<E6ProjectFilterTransposeOnlyWithWindow.Config> implements TransformationRule {

  /** Creates a ProjectFilterTransposeRule. */
  protected E6ProjectFilterTransposeOnlyWithWindow(Config config) {
    super(config);
  }
  ...
  @Override public void onMatch(RelOptRuleCall call) {
    ...
    if (call.rels.length >= 2) {
      ...
      if (filterChild instanceof HepRelVertex) {
        HepRelVertex hepRelVertex = (HepRelVertex) filterChild;
        filterChild = hepRelVertex.getCurrentRel();
      }

      if (!(filterChild instanceof Window)) {
        return;
      }
    } else {
      origProject = null;
    ...
    if (origProject != null && (config.isWholeProject() || config.isWholeFilter())) {
      builder.push(input);
      ...
      builder.project(
          replacer.visitList(origProject.getProjects()), origProject.getRowType().getFieldNames());
      ...
      final PushProjector pushProjector =
          new PushProjector(
              origProject, origFilter, input, config.preserveExprCondition(), builder);
  ...
  /**
   * Replaces whole expressions, or parts of an expression, with references to expressions computed
   * by an underlying Project.
   */
  private static class Replacer extends RexShuttle {
    ...
    @Override public void visitList(Iterable<? extends RexNode> exprs, List<RexNode> out) {
      for (RexNode expr : exprs) {
        out.add(visit(expr));
      }
    }
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT = ImmutableE6ProjectFilterTransposeOnlyWithWindow.Config.of();

    Config PROJECT = DEFAULT.withWholeProject(true);

    Config PROJECT_FILTER = PROJECT.withWholeFilter(true);

    @Override default E6ProjectFilterTransposeOnlyWithWindow toRule() {
      return new E6ProjectFilterTransposeOnlyWithWindow(this);
    }

    /** Expressions that should be preserved in the projection. */
    @Value.Default
    default PushProjector.ExprCondition preserveExprCondition() {
      return expr -> false;
    }
    ...
    /**
     * Whether to push whole expressions from the project; if false (the default), only pushes
     * references.
     */
    @Value.Default
    default boolean isWholeProject() {
      return false;
    }
    ...
    /**
     * Whether to push whole expressions from the filter; if false (the default), only pushes
     * references.
     */
    @Value.Default
    default boolean isWholeFilter() {
      return false;
    }
