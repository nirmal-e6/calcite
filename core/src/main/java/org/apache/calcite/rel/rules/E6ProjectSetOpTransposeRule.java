...
public class E6ProjectSetOpTransposeRule extends RelRule<E6ProjectSetOpTransposeRule.Config>
  ...
  @Override public void onMatch(RelOptRuleCall call) {
    ...
    Pair<Pair<List<RexNode>, List<Integer>>, LogicalProject> constantAndProjectPair =
        getAllConstantsAndUpdatedProjection(origProject);
    ...
    final PushProjector pushProjector =
        new PushProjector(newProject, null, setOp, config.preserveExprCondition(), call.builder());
  ...
  private Pair<Pair<List<RexNode>, List<Integer>>, LogicalProject>
      getAllConstantsAndUpdatedProjection(LogicalProject originalProject) {
    ...
    if (listOfConstants.isEmpty()) {
      ...
      return new Pair<>(
          new Pair<>(listOfConstants, indexesOfConstants),
          LogicalProject.create(
              originalProject.getInput(),
              originalProject.getHints(),
              newRexNodeList,
              newFieldNames));
    }
  }
  ...
  private LogicalProject removeConstantsFromLogicalProject(LogicalProject originalProject) {
    ...
    return LogicalProject.create(
        originalProject.getInput(), originalProject.getHints(), newRexNodeList, newFieldNames);
  }
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT =
        ImmutableE6ProjectSetOpTransposeRule.Config.builder()
