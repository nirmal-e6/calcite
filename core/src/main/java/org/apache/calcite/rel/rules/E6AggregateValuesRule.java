 ...
 *
 * <p>This is still useful because {@link PruneEmptyRules#AGGREGATE_INSTANCE} doesn't handle {@code
 * Aggregate}, which is in turn because {@code Aggregate} of empty relations need some special
 * handling: a single row will be generated, where each column's value depends on the specific
 * aggregate calls (e.g. COUNT is 0, SUM is NULL).
 *
 ...
 * </blockquote>
 *
 * <p>This rule only applies to "grand totals", that is, {@code GROUP BY ()}. Any non-empty {@code
 * GROUP BY} clause will return one row per group key value, and each group will consist of at least
 * one row.
 *
 * @see CoreRules#AGGREGATE_VALUES
 */
@Value.Enclosing
public class E6AggregateValuesRule extends RelRule<E6AggregateValuesRule.Config>
  ...
  public interface Config extends RelRule.Config {
    ...
    E6AggregateValuesRule.Config DEFAULT =
        ImmutableE6AggregateValuesRule.Config.of().withOperandFor(Aggregate.class, Values.class);

    @Override default E6AggregateValuesRule toRule() {
      return new E6AggregateValuesRule(this);
    }

    /** Defines an operand tree for the given classes. */
    default E6AggregateValuesRule.Config withOperandFor(
        Class<? extends Aggregate> aggregateClass, Class<? extends Values> valuesClass) {
                      ...
                      .oneInput(
                          b1 ->
                              b1.operand(valuesClass)
                                  ...
                                  .noInputs()))
          .as(E6AggregateValuesRule.Config.class);
    }
  }
}
