...
/**
 * Planner rule that slices a {@link Project} into sections which contain windowed aggregate
 * functions and sections which do not.
 *
 * <p>The sections which contain windowed agg functions become instances of {@link LogicalWindow}.
 * If the {@link LogicalCalc} does not contain any windowed agg functions, does nothing.
 *
 * <p>There is also a variant that matches {@link Calc} rather than {@code Project}.
 */
public abstract class E6ProjectToWindowRule extends RelRule<E6ProjectToWindowRule.Config>
  ...
  /**
   * Instance of the rule that applies to a {@link Calc} that contains windowed aggregates and
   * converts it into a mixture of {@link LogicalWindow} and {@code Calc}.
   *
  ...
  public static class E6CalcToWindowRule extends E6ProjectToWindowRule {
    ...
    public interface E6CalcToWindowRuleConfig extends E6ProjectToWindowRule.Config {
      E6CalcToWindowRuleConfig DEFAULT =
          ImmutableE6CalcToWindowRuleConfig.of()
              .withOperandSupplier(
  ...
  /**
   * Instance of the rule that can be applied to a {@link Project} and that produces, in turn, a
   * mixture of {@code LogicalProject} and {@link LogicalWindow}.
   *
   * @see CoreRules#PROJECT_TO_LOGICAL_PROJECT_AND_WINDOW
   */
  public static class E6ProjectToLogicalProjectAndWindowRule extends E6ProjectToWindowRule {

    private final Logger LOG =
        LoggerFactory.getLogger(E6ProjectToLogicalProjectAndWindowRule.class);
    ...
    public E6ProjectToLogicalProjectAndWindowRule(RelBuilderFactory relBuilderFactory) {
      this(
    ...
    @Override public void onMatch(RelOptRuleCall call) {
      ...
      final E6CalcRelSplitter transform =
          new WindowedAggRelSplitter(calc, call.builder(), project.getRowType().getFieldNames()) {
    ...
    /** Rule configuration. */
    @Value.Immutable
    public interface E6ProjectToLogicalProjectAndWindowRuleConfig
        extends E6ProjectToWindowRule.Config {
      E6ProjectToLogicalProjectAndWindowRuleConfig DEFAULT =
  ...
  static class WindowedAggRelSplitter extends E6CalcRelSplitter {
    private static final RelType[] REL_TYPES = {
        new RelType("CalcRelType") {
          ...
          @Override protected RelNode makeRel(
