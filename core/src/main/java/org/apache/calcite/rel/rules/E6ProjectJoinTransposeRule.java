...
/**
 * Planner rule that pushes a {@link Project} past a {@link Join} by splitting the projection into a
 * projection on top of each child of the join.
 *
 * @see CoreRules#PROJECT_JOIN_TRANSPOSE
 */
@Value.Enclosing
public class E6ProjectJoinTransposeRule extends RelRule<E6ProjectJoinTransposeRule.Config>
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT =
        ImmutableE6ProjectJoinTransposeRule.Config.builder()
                ...
                expr -> {
                  ...
                  if (SqlKind.CAST == expr.getKind()) {
                    ...
                    if (operand0Type.isNullable() && !relType.isNullable()) {
                      // Do not push down any CAST that changes nullability (nullable source -> NOT
                      // NULL target).
                      // When pushed through a LEFT/RIGHT/FULL JOIN, the column becomes nullable
                      // again,
                      // causing a type mismatch in verifyTypeEquivalence. This applies regardless
                      // of
