 ...
 * {@link Calc} with multiple {@link RexCall}
 ...
 */
public abstract class E6CalcRelSplitter
{
   ...
   */
  E6CalcRelSplitter(Calc calc, RelBuilder relBuilder, RelType[] relTypes, List<String> parentFieldNames) {
  ...
  private String deriveFieldName(RexNode expr, int ordinal) {
    ...
    } else if(parentFieldNames.size() > ordinal) {
      String fieldName = parentFieldNames.get(ordinal);
      if (!fieldName.startsWith("$") || fieldName.startsWith("$EXPR")) {
        return fieldName;
      }
    }
