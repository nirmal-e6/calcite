...
// shaded to use old timestampStringToUnixDate method from version 1.22.0
// this was affecting Fw Experimental HBO
// because of not simplifying timestamp string to timestamp, filter condition was altering
// eventually not able to use HBO hash
 ...
 *
 * <p>TODO: review methods for performance. Due to allocations required, it may be preferable to
...
public class DateTimeUtils {
  ...
  /**
   * Calendar set to the epoch (1970-01-01 00:00:00 UTC). Useful for initializing other values.
   * Calendars are not immutable, so be careful not to screw up this object for everyone else.
   */
  ...
  private static Calendar parseDateFormat(
      String s, DateFormat dateFormat, TimeZone tz, ParsePosition pp) {
    if (tz == null) {
      tz = DEFAULT_ZONE;
    }
   ...
   * @param tz time zone in which to interpret string. Defaults to the Java default time zone
   * @return a Calendar initialized with the parsed value, or null if parsing failed. If returned,
   *     the Calendar is configured to the UTC time zone.
   */
  ...
  public static PrecisionTime parsePrecisionDateTimeLiteral(String s, String pattern, TimeZone tz) {
    assert pattern != null;
    return parsePrecisionDateTimeLiteral(s, new SimpleDateFormat(pattern, Locale.ROOT), tz, 3);
  }
   ...
   * @return a {@link PrecisionTime PrecisionTime} initialized with the parsed value, or null if
   *     parsing failed. The PrecisionTime contains a GMT Calendar and a precision.
   */
  ...
  /**
   * Creates a new date formatter with Farrago specific options. Farrago parsing is strict and does
   * not allow values such as day 0, month 13, etc.
   *
  ...
  /**
   * Rounds a dividend to the nearest divisor. For example roundUp(31, 10) yields 30; roundUp(37,
   * 10) yields 40.
   *
  ...
  public static int dateStringToUnixDate(String s) {
    ...
    if (hyphen1 < 0) {
      ...
      y = parseInt(s.substring(0, hyphen1).trim());
      final int hyphen2 = s.indexOf('-', hyphen1 + 1);
      if (hyphen2 < 0) {
        m = parseInt(s.substring(hyphen1 + 1).trim());
        d = 1;
      } else {
        m = parseInt(s.substring(hyphen1 + 1, hyphen2).trim());
        d = parseInt(s.substring(hyphen2 + 1).trim());
      }
    }
  ...
  public static int timeStringToUnixDate(String v, int start) {
    ...
    int milli;
    if (colon1 < 0) {
      hour = parseInt(v.trim());
      minute = 0;
      second = 0;
      milli = 0;
    } else {
      hour = parseInt(v.substring(start, colon1).trim());
      final int colon2 = v.indexOf(':', colon1 + 1);
      if (colon2 < 0) {
        minute = parseInt(v.substring(colon1 + 1).trim());
        second = 0;
        milli = 0;
      } else {
        minute = parseInt(v.substring(colon1 + 1, colon2).trim());
        int dot = v.indexOf('.', colon2);
        if (dot < 0) {
          second = parseInt(v.substring(colon2 + 1).trim());
          milli = 0;
        } else {
          second = parseInt(v.substring(colon2 + 1, dot).trim());
  ...
  /**
   * Lenient date validation. This accepts more date strings than validateDate: it does not insist
   * on having two-digit values for days and months, and accepts spaces around the value.
   *
  ...
  /**
   * Returns the number of days in a month in the proleptic Gregorian calendar used by ISO-8601.
   *
   * <p>"Proleptic" means that we apply the calendar to dates before the Gregorian calendar was
   * invented (in 1582). Thus, years 0 and 1200 are considered leap years, and 1500 is not.
   */
  ...
  /** Whether a year is considered a leap year in the proleptic Gregorian calendar. */
  ...
  private static IllegalArgumentException fieldOutOfRange(String field, String full) {
    return new IllegalArgumentException(
        "Value of " + field + " field is out of range in '" + full + "'");
  }

  private static IllegalArgumentException invalidType(String type, String full) {
    return new IllegalArgumentException("Invalid " + type + " value, '" + full + "'");
  }
  ...
  // E6data change
  // using code from version 1.22.0
  private static long timestampStringToUnixDate0(String s) {
    ...
    long t;
    if (space >= 0) {
      d = (long) dateStringToUnixDate(s.substring(0, space));
      t = (long) timeStringToUnixDate(s, space + 1);
    } else {
      d = (long) dateStringToUnixDate(s);
      t = 0L;
    }
  ...
  /**
   * Returns the first day of the first week of a year. Per ISO-8601 it is the Monday of the week
   * that contains Jan 4, or equivalently, it is a Monday between Dec 29 and Jan 4. Sometimes it is
   * in the year before the given year.
   */
  ...
  /**
   * Returns the ISO-8601 week number based on year, month, day. Per ISO-8601 it is the Monday of
   * the week that contains Jan 4, or equivalently, it is a Monday between Dec 29 and Jan 4.
  ...
  public static int unixTimestampExtract(TimeUnitRange range, long timestamp) {
    return unixTimeExtract(range, (int) Math.floorMod(timestamp, MILLIS_PER_DAY));
  }
   ...
   *
   * @param year Year (e.g. 2020 means 2020 CE, 1 means 1 CE, 0 means 1 BCE because there is no 0
   *     CE, -1 means 2 BCE, etc.)
  ...
  public static long addMonths(long timestamp, int m) {
    final long millis = Math.floorMod(timestamp, DateTimeUtils.MILLIS_PER_DAY);
    timestamp -= millis;
  ...
  public static int subtractMonths(long t0, long t1) {
    ...
    final int d0 = (int) Math.floorDiv(t0 - millis0, DateTimeUtils.MILLIS_PER_DAY);
    final long millis1 = Math.floorMod(t1, DateTimeUtils.MILLIS_PER_DAY);
    final int d1 = (int) Math.floorDiv(t1 - millis1, DateTimeUtils.MILLIS_PER_DAY);
  ...
  /** Creates an instance of {@link Calendar} in the root locale and UTC time zone. */
   ...
   * date.
   *
   * <p>The {@link java.sql.Date} class uses the standard Gregorian calendar which switches from the
   * Julian calendar to the Gregorian calendar in October 1582. For compatibility with ISO-8601, the
   * value is converted to a {@link LocalDate} which uses the proleptic Gregorian calendar.
   *
  ...
  public static int sqlDateToUnixDate(java.sql.Date date, TimeZone timeZone) {
    ...
    long unixTimestamp =
        dateTime.toEpochSecond(ZoneOffset.UTC) * DateTimeUtils.MILLIS_PER_SECOND
   ...
   * Julian calendar to the Gregorian calendar in October 1582. For compatibility with ISO-8601, the
   * value is converted to a {@link LocalDateTime} which uses the proleptic Gregorian calendar.
   *
   * <p>For backwards compatibility, timezone offsets are calculated in relation to the local
  ...
  /**
   * Deals with values of {@code java.time.OffsetDateTime} without introducing a compile-time
   * dependency (because {@code OffsetDateTime} is only JDK 8 and higher).
   */
  ...
  /**
   * Implementation of {@code OffsetDateTimeHandler} for environments where no instances are
   * possible.
   */
