...
public class E6ConvertletTable extends ReflectiveConvertletTable {
  ...
  /**
   * Aggregate function kinds that the native executor handles natively. Using SqlKind-based
   * matching ensures alias operators (e.g. VARIANCE → VAR_SAMP, STDDEV → STDDEV_SAMP) are caught
   * regardless of which operator object the parser resolves to.
   */
  private static final Set<SqlKind> PRESERVE_AGG_KINDS =
      EnumSet.of(
          SqlKind.AVG,
          SqlKind.VAR_POP,
          SqlKind.VAR_SAMP,
  ...
  /**
   * Convertlet that preserves the aggregate call as-is (no expansion into SUM/COUNT). Builds a
   * RexCall with the original operator so that HistogramShuttle can wrap it in a RexOver with the
  ...
  private static final SqlRexConvertlet PASSTHROUGH_AGG_CONVERTLET =
      (cx, call) -> {
        ...
        return cx.getRexBuilder()
            .makeCall(cx.getValidator().getValidatedNodeType(call), call.getOperator(), exprs);
      };
  ...
  /**
   * Intercepts aggregate functions by SqlKind when running on the native executor, returning a
   * passthrough convertlet that prevents StandardConvertletTable's AvgVarianceConvertlet from
   * decomposing them into SUM/COUNT expressions.
   *
   * <p>For non-window aggregates, AggregateReduceFunctionsRule in the optimizer will still
   * decompose them if needed (controlled separately by CalciteForkSettings.nativeExecutor()).
   */
  ...
  protected RexNode convertVariantGet(SqlRexContext cx, final SqlCall call) {
    ...
    final List<RexNode> operands =
        convertOperands(
            cx, call, call.getOperandList().subList(0, 2), SqlOperandTypeChecker.Consistency.NONE);
    ...
    if (type.getSqlTypeName().equals(SqlTypeName.ANY)) {
      ...
      // In that case, we assume that no cast is happening and the cast type is the first operand's
      // type.
        ...
        new ArrayList<RexNode>() {
          {
            ...
            addAll(
                convertOperands(
                    cx,
                    call,
                    call.getOperandList().subList(0, 1),
            ...
            if (operandCount == 3) {
              ...
              addAll(
                  convertOperands(
                      cx,
                      call,
                      call.getOperandList().subList(2, 3),
                      SqlOperandTypeChecker.Consistency.NONE));
            }
          }
        };
  ...
  private RexNode convertCast(
      @UnknownInitialization E6ConvertletTable this, SqlRexContext cx, final SqlCall call) {
      ...
      if (nlsString != null) {
        ...
        left =
            SqlLiteral.createCharString(
                trimmed, nlsString.getCharsetName(), left.getParserPosition());
