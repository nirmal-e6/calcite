...
/**
 * E6 percentile aggregate variant used while rewriting ordered percentile
 * aggregates under window calls.
 */
public class E6PercentileCount extends SqlAggFunction {
  ...
  public E6PercentileCount(Optionality groupOrder) {
    super("PERCENTILE_CONT", null, SqlKind.PERCENTILE_CONT,
        PERCENTILE_CONT_RETURN_TYPE, null, UNIT_INTERVAL_NUMERIC_LITERAL_OR_ARRAY,
        SqlFunctionCategory.SYSTEM, true, false, groupOrder);
  }
  ...
  /**
   * Operand checker for a numeric literal in [0, 1], or an array constructor
   * containing numeric literals in [0, 1].
   */
  private static class UnitIntervalOrArrayTypeChecker
    ...
    @Override public boolean checkSingleOperandType(SqlCallBinding callBinding,
      ...
      if (operand instanceof SqlCall) {
        ...
        if (call.getKind() == SqlKind.ARRAY_VALUE_CONSTRUCTOR
          ...
          for (SqlNode elt : call.getOperandList()) {
            if (!checkUnitIntervalLiteralElement(callBinding, elt,
                throwOnFailure)) {
              return false;
            }
          }
      ...
      return UNIT_INTERVAL_NUMERIC_LITERAL.checkSingleOperandType(callBinding,
          operand, iFormalOperand, throwOnFailure);
    }

    private boolean checkUnitIntervalLiteralElement(SqlCallBinding callBinding,
        SqlNode operand, boolean throwOnFailure) {
      ...
      if (arg.getTypeName().getFamily() != SqlTypeFamily.NUMERIC) {
        if (throwOnFailure) {
          throw callBinding.newError(RESOURCE.argumentMustBeNumericLiteralInRange(
