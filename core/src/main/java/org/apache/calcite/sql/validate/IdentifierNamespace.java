...
/**
 * Namespace whose contents are defined by the type of an {@link org.apache.calcite.sql.SqlIdentifier identifier}.
 */
...
public IdentifierNamespace(SqlValidatorImpl validator, SqlNode node, @Nullable SqlNode enclosingNode,
...
protected static Pair<SqlIdentifier, @Nullable SqlNodeList> split(SqlNode node)
{
    switch (node.getKind())
    {
        case EXTEND:
            ...
            final SqlIdentifier identifier = operand0.getKind() == SqlKind.TABLE_REF ? ((SqlCall) operand0).operand(0)
    ...
    catch (CyclicDefinitionException e)
    {
        if (e.depth == 1)
        {
            throw validator.newValidationError(id,
                RESOURCE.cyclicDefinition(id.toString(), SqlIdentifier.getString(e.path)));
        }
        else
        {
    ...
    if (nameMatcher.isCaseSensitive())
    {
        ...
        if (resolved.count() == 1)
        {
            ...
            if (resolve.remainingNames.isEmpty() || previousResolve == null)
            {
                ...
                {
                    throw validator.newValidationError(id,
                        RESOURCE.objectNotFoundWithinDidYouMean(names.get(i), SqlIdentifier.getString(prefix), next));
                }
            }
            else
            {
...
public RelDataType validateImpl(RelDataType targetRowType)
{
    ...
    if (validator.config().identifierExpansion())
    {
        ...
        if (table != null)
        {
            ...
            {
                positions.add(
                    offset >= 0 && i >= offset ? id.getComponentParserPosition(i - offset) : id.getParserPosition());
            }
