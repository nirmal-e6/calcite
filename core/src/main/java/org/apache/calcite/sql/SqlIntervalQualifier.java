 ...
 *
 * <blockquote><code>
 * &lt;interval qualifier&gt; ::=<br> &nbsp;&nbsp; &lt;start field&gt; TO &lt;end field&gt;<br> &nbsp;&nbsp;| &lt;single
 * datetime field&gt;<br> &lt;start field&gt; ::=<br> &nbsp;&nbsp; &lt;non-second primary datetime field&gt;<br>
 * &nbsp;&nbsp; [ &lt;left paren&gt; &lt;interval leading field precision&gt; &lt;right paren&gt; ]<br> &lt;end
 * field&gt; ::=<br> &nbsp;&nbsp; &lt;non-second primary datetime field&gt;<br> &nbsp;&nbsp;| SECOND [ &lt;left
 * paren&gt; &lt;interval fractional seconds precision&gt; &lt;right paren&gt; ]<br> &lt;single datetime field&gt;
 * ::=<br> &nbsp;&nbsp;&lt;non-second primary datetime field&gt;<br> &nbsp;&nbsp;[ &lt;left paren&gt; &lt;interval
 * leading field precision&gt; &lt;right paren&gt; ]<br> &nbsp;&nbsp;| SECOND [ &lt;left paren&gt; &lt;interval leading
 * field precision&gt;<br> &nbsp;&nbsp;[ &lt;comma&gt; &lt;interval fractional seconds precision&gt; ] &lt;right
 * paren&gt; ]<br> &lt;primary datetime field&gt; ::=<br> &nbsp;&nbsp;&lt;non-second primary datetime field&gt;<br>
 * &nbsp;&nbsp;| SECOND<br> &lt;non-second primary datetime field&gt; ::= YEAR | MONTH | DAY | HOUR | MINUTE<br>
 * &lt;interval fractional seconds precision&gt; ::= &lt;unsigned integer&gt;<br> &lt;interval leading field
...
private static final Set<TimeUnitRange> TIME_UNITS = ImmutableSet.of(TimeUnitRange.HOUR, TimeUnitRange.MINUTE,
    TimeUnitRange.SECOND);

private static final Set<TimeUnitRange> MONTH_UNITS = ImmutableSet.of(TimeUnitRange.MILLENNIUM, TimeUnitRange.CENTURY,
    TimeUnitRange.DECADE, TimeUnitRange.YEAR, TimeUnitRange.ISOYEAR, TimeUnitRange.QUARTER, TimeUnitRange.MONTH);

private static final Set<TimeUnitRange> DAY_UNITS = ImmutableSet.of(TimeUnitRange.WEEK, TimeUnitRange.DAY);

private static final Set<TimeUnitRange> DATE_UNITS = ImmutableSet.<TimeUnitRange>builder()
    .addAll(MONTH_UNITS)
    .addAll(DAY_UNITS)
    .build();

private static final Set<String> WEEK_FRAMES = ImmutableSet.<String>builder()
...
private static final Set<String> TSI_TIME_FRAMES = ImmutableSet.of("SQL_TSI_FRAC_SECOND", "SQL_TSI_MICROSECOND",
    "SQL_TSI_SECOND", "SQL_TSI_MINUTE", "SQL_TSI_HOUR");

private static final Set<String> TSI_DATE_FRAMES = ImmutableSet.of("SQL_TSI_DAY", "SQL_TSI_WEEK", "SQL_TSI_MONTH",
...
private SqlIntervalQualifier(SqlParserPos pos, @Nullable String timeFrameName, TimeUnitRange timeUnitRange,
    int startPrecision, int fractionalSecondPrecision)
{
    super(pos);
...
public SqlIntervalQualifier(TimeUnit startUnit, int startPrecision, @Nullable TimeUnit endUnit,
    int fractionalSecondPrecision, SqlParserPos pos)
{
    this(pos, null, TimeUnitRange.of(requireNonNull(startUnit, "startUnit"), endUnit == startUnit ? null : endUnit),
...
public SqlIntervalQualifier(String timeFrameName, SqlParserPos pos)
{
    this(pos, requireNonNull(timeFrameName, "timeFrameName"),
        ...
        TimeUnitRange.EPOCH, RelDataType.PRECISION_NOT_SPECIFIED, RelDataType.PRECISION_NOT_SPECIFIED);
}
...
{
    return DATE_UNITS.contains(timeUnitRange) || timeFrameName != null && TSI_DATE_FRAMES.contains(timeFrameName)
        || isWeek();
}
...
{
    return TIME_UNITS.contains(timeUnitRange) || timeFrameName != null && TSI_TIME_FRAMES.contains(timeFrameName);
}
...
/**
 * Whether this qualifier represents {@code WEEK}, {@code ISOWEEK}, or {@code WEEK(}<i>weekday</i>{@code )} (for
 * <i>weekday</i> in {@code SUNDAY} .. {@code SATURDAY}).
 */
public boolean isWeek()
{
    return timeUnitRange == TimeUnitRange.WEEK || timeFrameName != null && WEEK_FRAMES.contains(timeFrameName);
}
...
public static int combineFractionalSecondPrecisionPreservingDefault(RelDataTypeSystem typeSystem,
    SqlIntervalQualifier qual1, SqlIntervalQualifier qual2)
{
    ...
    @SuppressWarnings("unused") TimeUnit unit)
{
    ...
    return startPrecision < POWERS10.length ? value.compareTo(POWERS10[startPrecision]) < 0
...
private static final BigDecimal[] POWERS10 = {ZERO, BigDecimal.valueOf(10), BigDecimal.valueOf(100),
    BigDecimal.valueOf(1000), BigDecimal.valueOf(10000), BigDecimal.valueOf(100000), BigDecimal.valueOf(1000000),
    BigDecimal.valueOf(10000000), BigDecimal.valueOf(100000000), BigDecimal.valueOf(1000000000),};
...
private static int[] fillYearMonthIntervalValueArray(int sign, BigDecimal year, BigDecimal month)
{
    int[] ret = new int[3];
...
private static int[] fillDayTimeIntervalValueArray(int sign, BigDecimal day, BigDecimal hour, BigDecimal minute,
 ...
 */
private int[] evaluateIntervalLiteralAsYear(RelDataTypeSystem typeSystem, int sign, String value, String originalValue,
    SqlParserPos pos)
{
    BigDecimal year;
 ...
 * @throws org.apache.calcite.runtime.CalciteContextException if the interval value is illegal
 */
private int[] evaluateIntervalLiteralAsMonth(RelDataTypeSystem typeSystem, int sign, String value, String originalValue,
    SqlParserPos pos)
{
    BigDecimal month;
 ...
 * @throws org.apache.calcite.runtime.CalciteContextException if the interval value is illegal
 */
private int[] evaluateIntervalLiteralAsQuarter(RelDataTypeSystem typeSystem, int sign, String value,
    String originalValue, SqlParserPos pos)
{
    BigDecimal quarter;
 ...
 * @throws org.apache.calcite.runtime.CalciteContextException if the interval value is illegal
 */
private int[] evaluateIntervalLiteralAsWeek(RelDataTypeSystem typeSystem, int sign, String value, String originalValue,
    SqlParserPos pos)
{
    BigDecimal week;
 ...
 * @throws org.apache.calcite.runtime.CalciteContextException if the interval value is illegal
 */
private int[] evaluateIntervalLiteralAsDay(RelDataTypeSystem typeSystem, int sign, String value, String originalValue,
    SqlParserPos pos)
{
    BigDecimal day;
    ...
    String originalValue, SqlParserPos pos)
{
    ...
    String intervalPatternWithFracSec = "(\\d+) (\\d{1,2}):(\\d{1,2}):(\\d{1,2})\\.(\\d{0," + fractionalSecondPrecision
        + "})";
    ...
    if (m.matches())
    {
        ...
        checkLeadFieldInRange(typeSystem, sign, day, TimeUnit.DAY, pos);
        if (!isSecondaryFieldInRange(hour, TimeUnit.HOUR) || !isSecondaryFieldInRange(minute, TimeUnit.MINUTE)
            || !isSecondaryFieldInRange(second, TimeUnit.SECOND) || !isFractionalSecondFieldInRange(secondFrac))
        {
            throw invalidValueException(pos, originalValue);
        }
 ...
 * @throws org.apache.calcite.runtime.CalciteContextException if the interval value is illegal
 */
private int[] evaluateIntervalLiteralAsHour(RelDataTypeSystem typeSystem, int sign, String value, String originalValue,
    SqlParserPos pos)
{
    BigDecimal hour;
    ...
    String originalValue, SqlParserPos pos)
{
    ...
    // Note: must check two patterns, since fractional second is optional
    final int fractionalSecondPrecision = getFractionalSecondPrecision(typeSystem);
    String intervalPatternWithFracSec = "(\\d+):(\\d{1,2}):(\\d{1,2})\\.(\\d{0," + fractionalSecondPrecision + "})";
    ...
    if (m.matches())
    {
        ...
        // Validate individual fields
        checkLeadFieldInRange(typeSystem, sign, hour, TimeUnit.HOUR, pos);
        if (!isSecondaryFieldInRange(minute, TimeUnit.MINUTE) || !isSecondaryFieldInRange(second, TimeUnit.SECOND)
    ...
    String originalValue, SqlParserPos pos)
{
    ...
    // Note: must check two patterns, since fractional second is optional
    final int fractionalSecondPrecision = getFractionalSecondPrecision(typeSystem);
    String intervalPatternWithFracSec = "(\\d+):(\\d{1,2})\\.(\\d{0," + fractionalSecondPrecision + "})";
...
/**
 * Validates an INTERVAL literal according to the rules specified by the interval qualifier. The assumption is made that
 * the interval qualifier has been validated prior to calling this method. Evaluating against an invalid qualifier could
...
{
    return SqlUtil.newContextException(pos, RESOURCE.unsupportedIntervalLiteral("'" + value + "'", "INTERVAL " + this));
}

private static CalciteContextException fieldExceedsPrecisionException(SqlParserPos pos, int sign, BigDecimal value,
...
/**
 * Converts a {@link SqlIntervalQualifier} to a {@link org.apache.calcite.sql.SqlIdentifier} if it is a time frame
 * reference.
 *
