...
public class E6SqlColonOperator extends SqlSpecialOperator {
  ...
  public static final SqlOperandTypeInference TYPE_INFERENCE =
      (callBinding, returnType, operandTypes) -> {
        // Get the first operand type
        SqlNode firstOperand = callBinding.operands().get(0);
        RelDataType firstOperandType =
            callBinding.getValidator().deriveType(callBinding.getScope(), firstOperand);
        ...
        RelDataType jsonPathType =
            callBinding.getValidator().deriveType(callBinding.getScope(), jsonPath);
        ...
        RelDataType castType =
            callBinding.getValidator().deriveType(callBinding.getScope(), castNode);
        if (castType.getSqlTypeName().equals(SqlTypeName.ANY)) {
          // We fall back to ANY type in the parser when there's no cast specified.
          // In that case, we assume that no cast is happening and the cast type is the first
          // operand's type.
  ...
  public static final SqlOperandTypeChecker TYPE_CHECKER =
      new SqlOperandTypeChecker() {
        private boolean ensureLiteral(
            SqlCallBinding callBinding, int position, boolean throwOnFailure) {
          if (!callBinding.isOperandLiteral(position, false)) {
            if (throwOnFailure) {
              throw callBinding
                  .getValidator()
                  .newValidationError(
                      callBinding.getCall(), RESOURCE.argumentMustBeLiteral("COLON"));
            }
            return false;
          }
          return true;
        ...
        @Override public boolean checkOperandTypes(SqlCallBinding callBinding,
          ...
          boolean isString =
              OperandTypes.STRING.checkSingleOperandType(
                  callBinding, callBinding.operand(0), 0, false);
          ...
          OperandTypes.STRING.checkSingleOperandType(
              callBinding, callBinding.operand(1), 0, throwOnFailure);
        ...
        createCall(
            SqlParserPos.sum(
                Arrays.asList(
                    ...
                    requireNonNull(thirdArg, "thirdArg").getParserPosition(),
                    list.pos(ordinal))),
            left,
            secondArg,
            thirdArg));
