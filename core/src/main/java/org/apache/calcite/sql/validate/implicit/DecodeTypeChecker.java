...
public class DecodeTypeChecker implements SqlOperandTypeChecker {
  ...
  private static boolean coerce(
      ...
      AbstractTypeCoercion abstractTypeCoercion,
      ImmutableList<RelDataType> dataTypes,
    ...
    for (int i = 0; i < expectedFamilies.size(); i++) {
      ...
      if (coeredType == null) {
        ...
        if ((SqlTypeUtil.isCharacter(in) || SqlTypeUtil.isNumeric(in))
