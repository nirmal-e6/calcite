 ...
 */
public static SqlReturnTypeInference andThen(UnaryOperator<SqlOperatorBinding> bindingTransform,
...
/**
 * Converts a binding of {@code FOO(x, y ORDER BY z)} or {@code FOO(x, y ORDER BY z SEPARATOR s)} to a binding of
 * {@code FOO(x, y)}. Used for {@code STRING_AGG} and {@code GROUP_CONCAT}.
 */
...
public static SqlCall stripOrderBy(SqlCall call)
{
    if (!call.getOperandList().isEmpty() && Util.last(call.getOperandList()) instanceof SqlNodeList)
    {
        ...
        return call.getOperator()
            .createCall(call.getFunctionQuantifier(), call.getParserPosition(), Util.skipLast(call.getOperandList()));
    }
    return call;
}
 ...
 * returned is the same as length of the first argument. If any of the other operands are nullable the returned type
 * will also be nullable. First Arg must be of string type.
 */
public static final SqlReturnTypeInference ARG0_NULLABLE_VARYING = ARG0.andThen(SqlTypeTransforms.TO_NULLABLE)
    .andThen(SqlTypeTransforms.TO_VARYING);

/**
 * Type-inference strategy whereby the result type of a call is the type of the operand #0 (0-based). If any of the
 * other operands are nullable the returned type will also be nullable.
 */
public static final SqlReturnTypeInference ARG0_NULLABLE = ARG0.andThen(SqlTypeTransforms.TO_NULLABLE);

/**
 * Type-inference strategy that determines the return type based on the first argument. If the first argument is an
 * array, the return type is consistent with {@link #ARG0_NULLABLE}. If the first argument is not an array, the return
 * type is consistent with {@link #ARG0_NULLABLE_VARYING}.
 */
...
/**
 * Type-inference strategy whereby the result type of a call is the type of the operand #0 (0-based). If the operand #0
 * (0-based) is nullable, the returned type will also be nullable.
 */
public static final SqlReturnTypeInference ARG0_NULLABLE_IF_ARG0_NULLABLE = ARG0.andThen(
    SqlTypeTransforms.ARG0_NULLABLE);

/**
 * Type-inference strategy whereby the result type of a call is the type of the operand #0 (0-based), with nulls always
 * allowed.
 */
...
public static final SqlReturnTypeInference ARG0_INTERVAL_NULLABLE = ARG0_INTERVAL.andThen(
...
/**
 * Type-inference strategy whereby the result type of a call is the type of the operand #1 (0-based). If any of the
 * other operands are nullable the returned type will also be nullable.
 */
...
/**
 * Type-inference strategy whereby the result type of a call is Boolean, with nulls allowed if any of the operands allow
 * nulls.
 */
public static final SqlReturnTypeInference BOOLEAN_NULLABLE = BOOLEAN.andThen(SqlTypeTransforms.TO_NULLABLE);

/**
 * Type-inference strategy whereby the result type of a call is Boolean, with nulls allowed if the type of the operand
 * #0 (0-based) is nullable.
 */
public static final SqlReturnTypeInference BOOLEAN_NULLABLE_IF_ARG0_NULLABLE = BOOLEAN.andThen(
    SqlTypeTransforms.ARG0_NULLABLE);
...
/**
 * Type-inference strategy that returns the type of the first operand, unless it is a DATE, in which case the return
 * type is TIMESTAMP. Supports cases such as <a href="https://issues.apache.org/jira/browse/CALCITE-5757">[CALCITE-5757]
 ...
 */
public static final SqlReturnTypeInference ARG0_EXCEPT_DATE_NULLABLE = ARG0_EXCEPT_DATE.andThen(
    SqlTypeTransforms.TO_NULLABLE);
 ...
 */
public static final SqlReturnTypeInference TIMESTAMP_LTZ = explicit(SqlTypeName.TIMESTAMP_WITH_LOCAL_TIME_ZONE);
 ...
 */
public static final SqlReturnTypeInference TIMESTAMP_LTZ_NULLABLE = TIMESTAMP_LTZ.andThen(
    SqlTypeTransforms.TO_NULLABLE);
...
/**
 * Type-inference strategy whereby the result type of a call is Double with nulls allowed if any of the operands allow
 * nulls.
 */
...
/**
 * Type-inference strategy whereby the result type of a call is a TINYINT with nulls allowed if any of the operands
 * allow nulls.
 */
...
/**
 * Type-inference strategy whereby the result type of a call is an Integer with nulls allowed if any of the operands
 * allow nulls.
 */
...
/**
 * Type-inference strategy whereby the result type of a call is a BIGINT with nulls allowed if any of the operands allow
 * nulls.
 */
 ...
 */
public static final SqlReturnTypeInference VARBINARY_FORCE_NULLABLE = VARBINARY.andThen(
...
/**
 * Type-inference strategy whereby the result type of a call is using its operands biggest type, using the SQL:1999
 * rules described in "Data types of results of aggregations". These rules are used in union, except, intersect, case
 * and other places.
 *
 * @see Glossary#SQL99 SQL:1999 Part 2 Section 9.3
 */
public static final SqlReturnTypeInference LEAST_RESTRICTIVE = andThen(SqlTypeTransforms.FROM_MEASURE_IF::apply,
...
/**
 * Type-inference strategy for NVL2 function. It returns the least restrictive type between the second and third
 * operands.
 */
public static final SqlReturnTypeInference NVL2_RESTRICTIVE = opBinding -> opBinding.getTypeFactory()
    .leastRestrictive(Arrays.asList(opBinding.getOperandType(1), opBinding.getOperandType(2)));

/**
 * Type-inference strategy that returns the type of the first operand, unless it is an integer type, in which case the
 ...
 * {@link SqlTypeTransforms#TO_NULLABLE}.
 */
public static final SqlReturnTypeInference ARG0_EXCEPT_INTEGER_NULLABLE = ARG0_EXCEPT_INTEGER.andThen(
    SqlTypeTransforms.TO_NULLABLE);
...
/**
 * Chooses a type to return. If all arguments are null, return nullable integer type. If all arguments are integer
 * types, choose the largest integer type. Nullable if any argument is nullable. As a fallback, choose the type of the
 * first argument that is not of the NULL type. Nullable if at least one argument is nullable.
 */
public static final SqlReturnTypeInference LARGEST_INT_OR_FIRST_NON_NULL = opBinding ->
{
    ...
    for (RelDataType opType : opBinding.collectOperandTypes())
    {
        ...
        if (SqlTypeName.INT_TYPES.contains(opType.getSqlTypeName()) && (largestIntegerType == null
    ...
    throw opBinding.newError(RESOURCE.atLeastOneArgumentMustNotBeNull(opBinding.getOperator().getName()));
};
...
{
    ExplicitOperatorBinding newBinding = new ExplicitOperatorBinding(opBinding, new AbstractList<RelDataType>()
    {
    ...
    return opBinding.getTypeFactory()
        .createMultisetType(
            requireNonNull(biggestElementType, () -> "can't infer element type for multiset of " + newBinding), -1);
};
 ...
 * <code>INTEGER</code>.
 */
public static final SqlReturnTypeInference TO_COLLECTION_ELEMENT = ARG0.andThen(
    SqlTypeTransforms.TO_COLLECTION_ELEMENT_TYPE);

public static final SqlReturnTypeInference TO_COLLECTION_ELEMENT_NULLABLE = TO_COLLECTION_ELEMENT.andThen(
    SqlTypeTransforms.TO_NULLABLE);

public static final SqlReturnTypeInference TO_COLLECTION_ELEMENT_FORCE_NULLABLE = TO_COLLECTION_ELEMENT.andThen(
    SqlTypeTransforms.FORCE_NULLABLE);
 ...
 */
public static final SqlReturnTypeInference MULTISET_ELEMENT_FORCE_NULLABLE = MULTISET.andThen(
    SqlTypeTransforms.TO_COLLECTION_ELEMENT_TYPE).andThen(SqlTypeTransforms.FORCE_NULLABLE);
...
/**
 * Same as {@link #IDENTITY_TO_MAP} but returns with nullability if any of the operands is nullable.
 */
public static final SqlReturnTypeInference IDENTITY_TO_MAP_NULLABLE = IDENTITY_TO_MAP.andThen(
    SqlTypeTransforms.TO_NULLABLE);
...
public static final SqlReturnTypeInference TO_MAP_ENTRIES_NULLABLE = TO_MAP_ENTRIES.andThen(
    SqlTypeTransforms.TO_NULLABLE);
...
public static final SqlReturnTypeInference TO_MAP_VALUES_NULLABLE = TO_MAP_VALUES.andThen(
    SqlTypeTransforms.TO_NULLABLE);
...
/**
 * Type-inference strategy for a call where the first argument is a decimal. The result type of a call is a decimal with
 * a scale of 0, and the same precision and nullability as the first argument.
 */
...
/**
 * Type-inference strategy that returns DECIMAL if any of the arguments are DECIMAL. It will return DOUBLE in all other
 * cases.
 */
...
public static final SqlReturnTypeInference DECIMAL_OR_DOUBLE_NULLABLE = DECIMAL_OR_DOUBLE.andThen(
    SqlTypeTransforms.TO_NULLABLE);

/**
 * Type-inference strategy whereby the result type of a call is {@link #DECIMAL_SCALE0} with a fallback to {@link #ARG0}
 ...
 * {@link SqlTypeTransforms#TO_NULLABLE}.
 */
public static final SqlReturnTypeInference DECIMAL_PRODUCT_NULLABLE = DECIMAL_PRODUCT.andThen(
    SqlTypeTransforms.TO_NULLABLE);
 ...
 */
public static final SqlReturnTypeInference PRODUCT_FORCE_NULLABLE = DECIMAL_PRODUCT_NULLABLE.orElse(LEAST_RESTRICTIVE)
 ...
 */
public static final SqlReturnTypeInference PRODUCT_NULLABLE = DECIMAL_PRODUCT_NULLABLE.orElse(ARG0_INTERVAL_NULLABLE)
    .orElse(LEAST_RESTRICTIVE);

/**
 * Type-inference strategy whereby the result type of a call is the decimal quotient of two exact numeric operands where
 * at least one of the operands is a decimal.
 */
 ...
 * {@link SqlTypeTransforms#TO_NULLABLE}.
 */
public static final SqlReturnTypeInference DECIMAL_QUOTIENT_NULLABLE = DECIMAL_QUOTIENT.andThen(
    SqlTypeTransforms.TO_NULLABLE);

/**
 * Type-inference strategy whereby the result type of a call is {@link #DOUBLE} if both operands are integer types.
 */
 ...
 * integers, not just decimals.
 */
public static final SqlReturnTypeInference QUOTIENT_FORCE_NULLABLE = DECIMAL_QUOTIENT_NULLABLE.orElse(LEAST_RESTRICTIVE)
    .andThen(SqlTypeTransforms.FORCE_NULLABLE);
 ...
 */
public static final SqlReturnTypeInference QUOTIENT_NULLABLE = DECIMAL_QUOTIENT_NULLABLE.orElse(ARG0_INTERVAL_NULLABLE)
    .orElse(LEAST_RESTRICTIVE);
 ...
 * not just decimals.
 */
public static final SqlReturnTypeInference SUM_FORCE_NULLABLE = DECIMAL_SUM_NULLABLE.orElse(LEAST_RESTRICTIVE)
    .andThen(SqlTypeTransforms.FORCE_NULLABLE);

/**
 * Type-inference strategy whereby the result type of a call is {@link #DECIMAL_SUM_NULLABLE} with a fallback to
 * {@link #LEAST_RESTRICTIVE} These rules are used for addition and subtraction.
 */
...
/**
 * Type-inference strategy whereby the result type of a call is the decimal modulus of two exact numeric operands where
 * at least one of the operands is a decimal.
 */
public static final SqlReturnTypeInference DECIMAL_MOD_NULLABLE = DECIMAL_MOD.andThen(SqlTypeTransforms.TO_NULLABLE);

/**
 * Type-inference strategy whereby the result type of a call is {@link #DECIMAL_MOD_NULLABLE} with a fallback to
 * {@link #ARG1_NULLABLE} These rules are used for modulus.
 */
...
public static final SqlReturnTypeInference DYADIC_STRING_SUM_PRECISION = opBinding ->
{
    ...
    final boolean containsAnyType = (argType0.getSqlTypeName() == SqlTypeName.ANY) || (argType1.getSqlTypeName()
        == SqlTypeName.ANY);

    final boolean containsNullType = (argType0.getSqlTypeName() == SqlTypeName.NULL) || (argType1.getSqlTypeName()
        == SqlTypeName.NULL);
    ...
    if (!containsAnyType && !containsNullType && SqlTypeUtil.inCharFamily(argType0))
    {
        ...
        pickedCollation = requireNonNull(
            SqlCollation.getCoercibilityDyadicOperator(getCollation(argType0), getCollation(argType1)),
    ...
    if (argType0.getPrecision() == RelDataType.PRECISION_NOT_SPECIFIED
        || argType1.getPrecision() == RelDataType.PRECISION_NOT_SPECIFIED || x > typeSystem.getMaxPrecision(typeName))
    {
    ...
    if (null != pickedCollation)
    {
        ...
        {
            throw new AssertionError("should never come here, " + "argType0=" + argType0 + ", argType1=" + argType1);
        }
        ret = typeFactory.createTypeWithCharsetAndCollation(ret, getCharset(pickedType), getCollation(pickedType));
    }
...
/**
 * Type-inference strategy for String concatenation. Result is varying if either input is; otherwise fixed. For
 * example,
 *
...
/**
 * Type-inference strategy for String concatenation with separator. The precision of separator should be calculated
 * during combining. Result is varying if either input is; otherwise fixed.
 *
 * <p>For example:
 *
 * <ul>
 ...
 */
public static final SqlReturnTypeInference MULTIVALENT_STRING_SUM_PRECISION_NULLABLE = MULTIVALENT_STRING_SUM_PRECISION.andThen(
    SqlTypeTransforms.TO_NULLABLE);

/**
 * Same as {@link #MULTIVALENT_STRING_SUM_PRECISION} and using {@link SqlTypeTransforms#TO_NOT_NULLABLE}.
 */
public static final SqlReturnTypeInference MULTIVALENT_STRING_SUM_PRECISION_NOT_NULLABLE = MULTIVALENT_STRING_SUM_PRECISION.andThen(
 ...
 */
public static final SqlReturnTypeInference MULTIVALENT_STRING_WITH_SEP_SUM_PRECISION_NOT_NULLABLE = MULTIVALENT_STRING_WITH_SEP_SUM_PRECISION.andThen(
    SqlTypeTransforms.TO_NOT_NULLABLE);

/**
 * Same as {@link #MULTIVALENT_STRING_WITH_SEP_SUM_PRECISION} and using {@link SqlTypeTransforms#TO_NULLABLE_ALL}.
 */
public static final SqlReturnTypeInference MULTIVALENT_STRING_WITH_SEP_SUM_PRECISION_ARG0_NULLABLE = MULTIVALENT_STRING_WITH_SEP_SUM_PRECISION.andThen(
    SqlTypeTransforms.ARG0_NULLABLE);

/**
 * Same as {@link #MULTIVALENT_STRING_SUM_PRECISION} and using {@link SqlTypeTransforms#TO_NULLABLE_ALL}.
 */
public static final SqlReturnTypeInference MULTIVALENT_STRING_SUM_PRECISION_NULLABLE_ALL = MULTIVALENT_STRING_SUM_PRECISION.andThen(
 ...
 */
public static final SqlReturnTypeInference DYADIC_STRING_SUM_PRECISION_NULLABLE_VARYING = DYADIC_STRING_SUM_PRECISION.andThen(
    SqlTypeTransforms.TO_NULLABLE).andThen(SqlTypeTransforms.TO_VARYING);

/**
 * Same as {@link #DYADIC_STRING_SUM_PRECISION} and using {@link SqlTypeTransforms#TO_NULLABLE}.
 */
public static final SqlReturnTypeInference DYADIC_STRING_SUM_PRECISION_NULLABLE = DYADIC_STRING_SUM_PRECISION.andThen(
    SqlTypeTransforms.TO_NULLABLE);
...
{
    final RelDataTypeFactory typeFactory = opBinding.getTypeFactory();
    final RelDataType type = typeFactory.getTypeSystem().deriveSumType(typeFactory, opBinding.getOperandType(0));
    if (opBinding.getGroupCount() == 0 || opBinding.hasFilter())
    {
...
{
    final RelDataTypeFactory typeFactory = opBinding.getTypeFactory();
    final RelDataType sumType = typeFactory.getTypeSystem().deriveSumType(typeFactory, opBinding.getOperandType(0));
...
public static final SqlReturnTypeInference AVG_AGG_FUNCTION = opBinding ->
{
    ...
    if (opBinding.getGroupCount() == 0 || opBinding.hasFilter() || opBinding.getOperator().kind == SqlKind.STDDEV_SAMP)
    {
