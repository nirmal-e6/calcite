...
// Shaded class for following E6data fixes and Impls
// subquery recognition, star expansion guardrail, ambiguous alias fix, alias in agg issue, Except and TOP clause support
// and made changes to support pivot column renaming according to snowflake syntax & custom LambdaScope
...
// variable isSubQuery is added by E6Data
protected boolean isSubQuery = false;
protected boolean m_bHasUsingClause = false;

/**
 * Maps {@link SqlParserPos} strings to the {@link SqlIdentifier} identifier objects at these positions.
 */
...
/**
 * Maps a {@link SqlNode node} to the {@link SqlValidatorNamespace namespace} which describes what columns they
 * contain.
 */
protected final IdentityHashMap<SqlNode, SqlValidatorNamespace> namespaces = new IdentityHashMap<>();

/**
 * Set of select expressions used as cursor definitions. In standard SQL, only the top-level SELECT is a cursor; Calcite
 * extends this with cursors as inputs to table functions.
 */
private final Set<SqlNode> cursorSet = Sets.newIdentityHashSet();

/**
 * Stack of objects that maintain information about function calls. A stack is needed to handle nested function calls.
 * The function call currently being validated is at the top of the stack.
 */
...
/**
 * Map of derived RelDataType for each node. This is an IdentityHashMap since in some cases (such as null literals) we
 * need to discriminate by instance.
 */
...
private final Map<String, RelDataType> pivotRenamingColumns = new LinkedHashMap<>();
private boolean hasPivotWithStar = false;
    ...
    RelDataTypeFactory typeFactory, Config config)
{
    ...
    @SuppressWarnings("argument.type.incompatible") TypeCoercion typeCoercion = config.typeCoercionFactory()
        .create(typeFactory, this);
    this.typeCoercion = typeCoercion;

    if (config.conformance().allowLenientCoercion())
    {
        final SqlTypeCoercionRule rules = first(config.typeCoercionRules(), SqlTypeCoercionRule.instance());

        final ImmutableSet<SqlTypeName> arrayMapping = ImmutableSet.<SqlTypeName>builder()
...
public SqlNodeList expandStar(SqlNodeList selectList, SqlSelect select, boolean includeSystemVars)
{
    ...
    for (final SqlNode selectItem : selectList)
    {
        ...
        expandSelectItem(selectItem, select, first(originalType, unknownType), list,
            catalogReader.nameMatcher().createSet(), types, includeSystemVars);
    }
...
private boolean expandSelectItem(final SqlNode selectItem, SqlSelect select, RelDataType targetType,
    List<SqlNode> selectItems, Set<String> aliases, PairList<String, RelDataType> fields, boolean includeSystemVars)
{
    ...
    else
    {
        ...
        {
            // E6data Guardrail - Star Expansion Rule
            CalciteForkSettings.checkStarExpansion(selectItems.size());
            return true;
        }

        selectScope = getSelectScope(select);
        // if condition added by E6data to expand pivot renaming columns
        if (select.getFrom() instanceof SqlBasicCall
            && ((SqlBasicCall) select.getFrom()).getOperator().getKind() == SqlKind.AS
            && ((SqlBasicCall) select.getFrom()).getOperandList().get(0) instanceof SqlPivot)
        {
            SqlPivot pivot = (SqlPivot) ((SqlBasicCall) select.getFrom()).getOperandList().get(0);
            expanded = expandPivotSelectExpr(selectItem, scope, select, (PivotNamespace) getNamespaceOrThrow(pivot));
        }
        else
        {
    ...
    // E6data fix for ambiguous alias
    if (!CalciteForkSettings.allowDuplicateAliasInProjection() &&
        aliases.contains(alias) && (!(expanded instanceof SqlCall
        && (((SqlCall) expanded).getOperator() instanceof SqlAsOperator))))
    {
        // generate ordinal alias
        // it will generate EXPR$1 like alias
        // as Calcite identifies this as generated alias and uses
        // correctly than if we try to generate <column_name>1 (e.g cc_city1)
        ...
        //        alias = String.format("%s%s", alias, aliases.size());
    }
    ...
    for (String name : SqlIdentifier.simpleNames((SqlNodeList) getCondition(join)))
    {
        if (identifier.getSimple().equals(name))
        {
            ...
            for (ScopeChild child : requireNonNull(scope, "scope").children)
            {
                ...
                {
                    final SqlIdentifier exp = new SqlIdentifier(ImmutableList.of(child.name, name),
...
private static Map<String, String> getFieldAliases(final SelectScope scope)
{
    ...
    for (SqlNode selectItem : scope.getNode().getSelectList())
    {
        if (selectItem instanceof SqlCall)
        {
            ...
            if (!(call.getOperator() instanceof SqlAsOperator) || !(call.operand(0) instanceof SqlIdentifier))
            {
                continue;
            }
            ...
            final SqlIdentifier fieldIdentifier2 = call.operand(1);
            fieldAliases.put(fieldIdentifier1.names.get(fieldIdentifier1.names.size() - 1),
                fieldIdentifier2.names.get(fieldIdentifier2.names.size() - 1));
        }
    }
...
/**
 * Returns the set of field names in the join condition specified by USING or implicitly by NATURAL, de-duplicated and
 * in order.
 */
...
{
    return SqlValidatorUtil.deriveNaturalJoinColumnList(catalogReader.nameMatcher(),
        getNamespaceOrThrow(join.getLeft()).getRowType(), getNamespaceOrThrow(join.getRight()).getRowType());
}

private static SqlNode expandCommonColumn(SqlSelect sqlSelect, SqlNode selectItem, SelectScope scope,
...
private static void validateQualifiedCommonColumn(SqlJoin join, SqlIdentifier identifier, SelectScope scope,
    SqlValidatorImpl validator)
{
...
private boolean expandStar(List<SqlNode> selectItems, Set<String> aliases, PairList<String, RelDataType> fields,
    boolean includeSystemVars, SelectScope scope, SqlNode node)
{
    ...
    switch (identifier.names.size())
    {
        case 1:
            ...
            for (ScopeChild child : scope.children)
            {
                ...
                {
                    // change in whole else statement by E6data for Pivot
                    final SqlNode from2 = SqlNonNullableAccessors.getNode(child);
                    if (from2 instanceof SqlPivot)
                    {
                        hasPivotWithStar = true;
                    }
                    final SqlValidatorNamespace fromNs = getNamespaceOrThrow(from2, scope);
                    final RelDataType rowType = fromNs.getRowType();
                    int currentRenamingColumnIndex = 0;

                    for (RelDataTypeField field : rowType.getFieldList())
                    {
                        String columnName = field.getName();
                        SqlIdentifier exp = null;
                        if (pivotRenamingColumns.isEmpty())
                        {
                            // TODO: do real implicit collation here
                            exp = new SqlIdentifier(
                        ...
                        else
                        {
                            if (from2 instanceof SqlPivot)
                            {
                                SqlPivot pivot = (SqlPivot) from2;
                                List<SqlNode> inList = pivot.inList.getList();
                                for (SqlNode inListNode : inList)
                                {
                                    if (inListNode.toString().equals(columnName))
                                    {
                                        exp = new SqlIdentifier(ImmutableList.of(child.name, columnName),
                                            startPosition);
                                        break;
                                    }
                                }
                                ...
                                {
                                    SqlNode nodeX = exp;
                                    if (currentRenamingColumnIndex < pivotRenamingColumns.size())
                                    {
                                        nodeX = new SqlBasicCall(SqlStdOperatorTable.AS, ImmutableList.of(exp, new SqlIdentifier(
                                            pivotRenamingColumns.keySet()
                                            .stream()
                                            .collect(Collectors.toList())
                                            .get(currentRenamingColumnIndex), exp.getParserPosition())),
                                            exp.getParserPosition());
                                        currentRenamingColumnIndex++;
                                    }

                                    if (!isRolledUpColumn(exp, scope))
                                    {
                                        addToSelectList(selectItems, aliases, fields, nodeX, scope, includeSystemVars);
                                    }
                                }
                            }
                            ...
                            if (exp == null)
                            {
                                exp = new SqlIdentifier(columnName, startPosition);

                                SqlNode nodeX = exp;
                                if (currentRenamingColumnIndex < pivotRenamingColumns.size())
                                {
                                    nodeX = new SqlBasicCall(SqlStdOperatorTable.AS, ImmutableList.of(exp, new SqlIdentifier(
                                        pivotRenamingColumns.keySet().stream().collect(Collectors.toList()).get(currentRenamingColumnIndex),
                                        exp.getParserPosition())), exp.getParserPosition());
                                    currentRenamingColumnIndex++;
                                }
                                ...
                                if (!isRolledUpColumn(exp, scope))
                                {
                                    addToSelectList(selectItems, aliases, fields, nodeX,
                                        (from2 instanceof SqlPivot) ? scopes.get(from2) : scope, includeSystemVars);
                                }
                            }
                        }
                    }
                ...
                if (child.nullable)
                {
                    for (int i = before; i < fields.size(); i++)
                    {
                        ...
                        if (!type.isNullable())
                        {
                            fields.set(i, entry.getKey(), typeFactory.createTypeWithNullability(type, true));
                        }
                    }
                }
            }
        ...
        default:
            ...
            final SqlNameMatcher resolvedNameMatcher = scope.validator.catalogReader.nameMatcher();
            ...
            {
                // don't expand star if the underneath table is dynamic.
                addToSelectList(selectItems, aliases, fields,
                    prefixId.plus(DynamicRecordType.DYNAMIC_STAR_PREFIX, startPosition), scope, includeSystemVars);
            }
...
{
    return SqlTypeUtil.equalSansNullability(typeFactory, currentType, desiredType) ? node
...
private boolean addOrExpandField(List<SqlNode> selectItems, Set<String> aliases, PairList<String, RelDataType> fields,
    boolean includeSystemVars, SelectScope scope, SqlIdentifier id, RelDataTypeField field)
{
...
private void lookupFromHints(@Nullable SqlNode node, SqlValidatorScope scope, SqlParserPos pos,
...
private void lookupJoinHints(SqlJoin join, SqlValidatorScope scope, SqlParserPos pos, Collection<SqlMoniker> hintList)
{
    ...
    {
        case ON:
            requireNonNull(condition, () -> "join.getCondition() for " + join).findValidOptions(this,
                getScopeOrThrow(join), pos, hintList);
            return;
        default:
...
private static void findAllValidFunctionNames(List<String> names, SqlValidator validator, Collection<SqlMoniker> result,
    SqlParserPos pos)
{
...
protected void validateNamespace(final SqlValidatorNamespace namespace, RelDataType targetRowType)
{
    ...
    if (node != null)
    {
        ...
        if (node == top)
        {
            ...
            if (!filterRequirement.filterFields.isEmpty() || !filterRequirement.remnantFilterFields.isEmpty())
            {
                Stream<String> mustFilterStream = filterRequirement.filterFields.stream()
                    .mapToObj(namespace.getRowType().getFieldNames()::get);
                Stream<String> remnantStream = filterRequirement.remnantFilterFields.stream()
...
{
    return requireNonNull(clauseScopes.get(IdPair.of(select, clause)), () -> "no " + clause + " scope for " + select);
}
...
@Override
public SqlValidatorScope getFunctionScope(final SqlNode function)
{
    ...
    else if (node instanceof SqlCall)
    {
        ...
        switch (call.getOperator().getKind())
        {
            ...
            case EXTEND:
                ...
                final SqlIdentifier identifier = operand0.getKind() == SqlKind.TABLE_REF ? ((SqlCall) operand0).operand(
                    0) : (SqlIdentifier) operand0;
...
private @Nullable SqlValidatorNamespace getNamespace(SqlIdentifier id, @Nullable DelegatingScope scope)
{
    if (id.isSimple())
    {
        final SqlNameMatcher nameMatcher = catalogReader.nameMatcher();
        final SqlValidatorScope.ResolvedImpl resolved = new SqlValidatorScope.ResolvedImpl();
        requireNonNull(scope, () -> "scope needed to lookup " + id).resolve(id.names, nameMatcher, false, resolved);
        if (resolved.count() == 1)
        {
...
{
    return requireNonNull(getNamespace(node, scope), () -> "namespace for " + node + ", scope " + scope);
}
...
{
    return requireNonNull(getNamespace(id, scope), () -> "namespace for " + id + ", scope " + scope);
}
...
/**
 * Performs expression rewrites which are always used unconditionally. These rewrites massage the expression tree into a
 * standard form so that the rest of the validation logic can be simpler.
 *
...
protected @PolyNull SqlNode performUnconditionalRewrites(@PolyNull SqlNode node, boolean underFrom)
{
    ...
    if (node instanceof SqlCall)
    {
        ...
        if (call.getOperator() instanceof SqlUnresolvedFunction)
        {
            ...
            opTab.lookupOperatorOverloads(function.getNameAsId(), function.getFunctionType(), SqlSyntax.FUNCTION,
                overloads, catalogReader.nameMatcher());

            // condition changed by E6data
            // original condition  => [ overloads.size() == 1 ]
            // pick first function to set operator, so it doesn't set as user defined
            // picking first one because if 2 instances are there then first one will be from the custom table
            boolean isCustomOperatorTable = CalciteForkSettings.isCustomOperatorTable(opTab)
                || opTab instanceof ChainedSqlOperatorTable;
            if (overloads.size() == 1 || (isCustomOperatorTable && overloads.size() == 2))
            {
    ...
    switch (kind)
    {
        case OVER:
        {
            SqlCall aggCall = ((SqlCall) node).operand(0);
            SqlWindow window = ((SqlCall) node).operand(1);
            if (aggCall.getKind() == SqlKind.WITHIN_GROUP)
            {
                SqlCall actualAgg = aggCall.operand(0);
                SqlNodeList orderByList = (SqlNodeList) aggCall.getOperandList().get(1);
                switch (actualAgg.getKind())
                {
                    case PERCENTILE_CONT:
                        window.setOrderList(orderByList);
                        E6PercentileCount function = new E6PercentileCount(Optionality.FORBIDDEN);
                        actualAgg = function.createCall(actualAgg.getParserPosition(), actualAgg.getOperandList());
                        actualAgg.getOperator();
                        ((SqlCall) node).setOperand(0, actualAgg);
                        return node;
                    default:
                        // DO NOTHING, will fail most probably
                }
            }
        }
        case VALUES:
        ...
        case ORDER_BY:
        {
            ...
            if (orderBy.query instanceof SqlSelect)
            {
                ...
                // E6data exception check for TOP clause
                if (select.getFetch() != null && orderBy.fetch != null)
                {
                    throw new CalciteException("TOP and LIMIT can't be used together!", new Throwable());
                }
                ...
                if (select.getOrderList() == null)
                {
                    ...
                    // E6data TOP clause bug fix for CTEs
            ...
            if (innerSelect != null && isAggregate(innerSelect))
            {
                ...
                for (int i = 0; i < orderList.size(); i++)
                {
                    ...
                    for (Ord<SqlNode> sel : Ord.zip(selectList2))
                    {
                        ...
                        {
                            orderList.set(i,
                                SqlLiteral.createExactNumeric(Integer.toString(sel.i + 1), SqlParserPos.ZERO));
                        }
                    }
                }
            }
            ...
            return new SqlSelect(SqlParserPos.ZERO, null, selectList, null, orderBy.query, null, null, null, null, null,
                orderList, orderBy.offset, orderBy.fetch, null);
        }
        ...
        case EXPLICIT_TABLE:
        {
            ...
            final SqlNodeList selectList = new SqlNodeList(SqlParserPos.ZERO);
            selectList.add(SqlIdentifier.star(SqlParserPos.ZERO));
            return new SqlSelect(SqlParserPos.ZERO, null, selectList, null, call.operand(0), null, null, null, null,
...
private static void rewriteMerge(SqlMerge call)
{
    ...
    SqlNode outerJoin = new SqlJoin(SqlParserPos.ZERO, leftJoinTerm, SqlLiteral.createBoolean(false, SqlParserPos.ZERO),
        joinType.symbol(SqlParserPos.ZERO), targetTable, JoinConditionType.ON.symbol(SqlParserPos.ZERO),
        call.getCondition());
    SqlSelect select = new SqlSelect(SqlParserPos.ZERO, null, selectList, null, outerJoin, null, null, null, null, null,
        null, null, null, null);
    call.setSourceSelect(select);
    ...
    if (insertCall != null)
    {
        ...
        select = new SqlSelect(SqlParserPos.ZERO, null, selectList, null, insertSource, null, null, null, null, null,
            null, null, null, null);
...
private void expandMergeActionStars(SqlMerge call)
{
    ...
    SqlInsert insertCall = call.getInsertCall();
    final boolean updateStar = updateCall != null && isUpdateStar(updateCall);
    final boolean insertStar = insertCall != null && isInsertStar(insertCall);
    if (!updateStar && !insertStar)
    {
        return;
    }
    final SqlValidatorTable table = requireNonNull(getMergeTargetTable(call), () -> "target table for " + call);
    final SqlNodeList targetColumnList = createMergeStarTargetColumnList(table.getRowType());
    final SqlNodeList sourceExpressionList = createMergeStarSourceExpressionList(call, table.getRowType());
    ...
    if (insertStar)
    {
        ...
        final SqlNode row = SqlStdOperatorTable.ROW.createCall(sourceExpressionList.getParserPosition(),
            sourceExpressionList.toArray(new SqlNode[0]));
        insert.setOperand(2, SqlStdOperatorTable.VALUES.createCall(sourceExpressionList.getParserPosition(), row));
    }
}

private static boolean isUpdateStar(SqlUpdate updateCall)
{
    return isSingleStar(updateCall.getTargetColumnList()) && isSingleStar(updateCall.getSourceExpressionList());
}

private static boolean isInsertStar(SqlInsert insertCall)
{
...
{
    final @Nullable SqlIdentifier id = getMergeTargetTableIdentifier(call.getTargetTable());
    return id == null ? null : catalogReader.getTable(id.names);
}

private static @Nullable SqlIdentifier getMergeTargetTableIdentifier(SqlNode targetTable)
{
...
private static SqlNodeList createMergeStarTargetColumnList(RelDataType targetRowType)
{
    ...
    {
        targetColumnList.add(new SqlIdentifier(field.getName(), SqlParserPos.ZERO));
    }
    return targetColumnList;
}

private static SqlNodeList createMergeStarSourceExpressionList(SqlMerge call, RelDataType targetRowType)
{
    final SqlNodeList sourceExpressionList = new SqlNodeList(SqlParserPos.ZERO);
    final @Nullable String sourceAlias = SqlValidatorUtil.alias(call.getSourceTableRef());
    for (RelDataTypeField field : targetRowType.getFieldList())
    {
        sourceExpressionList.add(sourceAlias == null ? new SqlIdentifier(field.getName(), SqlParserPos.ZERO)
                                                     : new SqlIdentifier(
                                                         ImmutableList.of(sourceAlias, field.getName()),
                                                         SqlParserPos.ZERO));
    }
...
private SqlNode rewriteUpdateToMerge(SqlUpdate updateCall, SqlNode selfJoinSrcExpr)
{
    ...
    SqlNode selfJoinTgtExpr = getSelfJoinExprForUpdate(updateCall.getTargetTable(), updateAlias.getSimple());
    ...
    source = new SqlSelect(SqlParserPos.ZERO, null, selectList, null, source, null, null, null, null, null, null, null,
        null, null);
    source = SqlValidatorUtil.addAlias(source, UPDATE_SRC_ALIAS);
    SqlMerge mergeCall = new SqlMerge(updateCall.getParserPosition(), target, condition, source, updateCall, null, null,
        updateCall.getAlias());
...
/**
 * Allows a subclass to provide information about how to convert an UPDATE into a MERGE via self-join. If this method
 * returns null, then no such conversion takes place. Otherwise, this method should return a suitable unique identifier
 * expression for the given table.
 *
 * @param table identifier for table being updated
 * @param alias alias to use for qualifying columns in expression, or null for unqualified references; if this is
 *     equal to {@value #UPDATE_SRC_ALIAS}, then column references have been anonymized to "SYS$ANONx", where x is the
...
protected SqlSelect createSourceSelectForUpdate(SqlUpdate call)
{
    ...
    return new SqlSelect(SqlParserPos.ZERO, null, selectList, null, sourceTable, call.getCondition(), null, null, null,
        null, null, null, null, null);
}
...
public @Nullable SqlCall makeNullaryCall(SqlIdentifier id)
{
    if (!id.isComponentQuoted(id.names.size() - 1))
    {
        ...
        {
            if (operator.getSyntax() == SqlSyntax.FUNCTION_ID || operator.getSyntax() == SqlSyntax.FUNCTION_ID_CONSTANT)
            {
                // Even though this looks like an identifier, it is a
                // actually a call to a function. Construct a fake
                // call to this function, so we can use the regular
                // operator validation.
                SqlCall sqlCall = new SqlBasicCall(operator, ImmutableList.of(), id.getParserPosition(),
                    null).withExpanded(true);
                if (operator.getSyntax() == SqlSyntax.FUNCTION_ID_CONSTANT && !this.config.conformance()
...
public RelDataType deriveType(SqlValidatorScope scope, SqlNode expr)
{
    ...
    {
        inferUnknownTypes(unknownType, scope, expr);
    }
...
public RelDataType deriveConstructorType(SqlValidatorScope scope, SqlCall call, SqlFunction unresolvedConstructor,
    @Nullable SqlFunction resolvedConstructor, List<RelDataType> argTypes)
{
    ...
    if (config.identifierExpansion())
    {
        ...
        else
        {
            ...
            ((SqlBasicCall) call).setOperator(
                new SqlFunction(requireNonNull(type.getSqlIdentifier(), () -> "sqlIdentifier of " + type),
                    ReturnTypes.explicit(type), null, null, null, SqlFunctionCategory.USER_DEFINED_CONSTRUCTOR));
        }
    }
    return type;
}
...
@Override
public CalciteException handleUnresolvedFunction(SqlCall call, SqlOperator unresolvedFunction,
    List<RelDataType> argTypes, @Nullable List<String> argNames)
{
...
protected void inferUnknownTypes(RelDataType inferredType, SqlValidatorScope scope, SqlNode node)
{
    ...
    if ((node instanceof SqlDynamicParam) || isNullLiteral)
    {
        ...
        {
            newInferredType = typeFactory.createTypeWithCharsetAndCollation(newInferredType, getCharset(inferredType),
...
/**
 * Adds an expression to a select list, ensuring that its alias does not clash with any existing expressions on the
 * list.
 */
protected void addToSelectList(List<SqlNode> list, Set<String> aliases, List<Map.Entry<String, RelDataType>> fieldList,
    SqlNode exp, SqlValidatorScope scope, final boolean includeSystemVars)
{
...
private void registerMatchRecognize(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlMatchRecognize call,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable)
{
    ...
    SqlNode newExpr = registerFrom(usingScope, matchRecognizeScope, true, expr, expr, null, null, forceNullable, false);
    if (expr != newExpr)
    {
...
private void registerPivot(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlPivot pivot,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable)
{
    ...
    SqlNode newExpr = registerFrom(parentScope, scope, true, expr, expr, null, null, forceNullable, false);
    if (expr != newExpr)
    {
...
private void registerUnpivot(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlUnpivot call,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable)
{
...
/**
 * Registers a new namespace, and adds it as a child of its parent scope. Derived class can override this method to
 * tinker with namespaces as they are created.
 *
 ...
 */
protected void registerNamespace(@Nullable SqlValidatorScope usingScope, @Nullable String alias,
    SqlValidatorNamespace ns, boolean forceNullable)
{
    SqlValidatorNamespace namespace = namespaces.get(requireNonNull(ns.getNode(), () -> "ns.getNode() for " + ns));
    if (namespace == null)
    {
    ...
    if (usingScope != null)
    {
        if (alias == null)
        {
            throw new IllegalArgumentException(
                "Registering namespace " + ns + ", into scope " + usingScope + ", so alias must not be null");
        }
 ...
 * {@code enclosingNode} is the topmost node within the FROM clause, from which any decorations like an alias (<code>AS
 * alias</code>) or a table sample clause are stripped away to get {@code node}. Both are recorded in the namespace.
 *
 ...
 * @param lateral Whether LATERAL is specified, so that items to the left of this in the JOIN tree are visible in
 *     the scope
 *
...
private SqlNode registerFrom(SqlValidatorScope parentScope0, SqlValidatorScope usingScope, boolean register,
    final SqlNode node, SqlNode enclosingNode, @Nullable String alias, @Nullable SqlNodeList extendList,
    boolean forceNullable, final boolean lateral)
{
    final SqlKind kind = node.getKind();
    ...
    switch (kind)
    {
        case AS:
            ...
            expr = call.operand(0);
            final boolean needAliasNamespace = call.operandCount() > 2 || expr.getKind() == SqlKind.VALUES
                || expr.getKind() == SqlKind.UNNEST || expr.getKind() == SqlKind.COLLECTION_TABLE;
            newExpr = registerFrom(parentScope, usingScope, !needAliasNamespace, expr, enclosingNode, alias, extendList,
        ...
        case TABLESAMPLE:
            call = (SqlCall) node;
            expr = call.operand(0);
            newExpr = registerFrom(parentScope, usingScope, true, expr, enclosingNode, alias, extendList, forceNullable,
                lateral);
            if (newExpr != expr)
            {
                call.setOperand(0, newExpr);
        ...
        case JOIN:
            ...
            final SqlNode newLeft = registerFrom(parentScope, joinScope, true, left, left, null, null,
                forceLeftNullable, lateral);
            if (newLeft != left)
            {
                join.setLeft(newLeft);
            }
            final SqlNode newRight = registerFrom(parentScope, joinScope, true, right, right, null, null,
                forceRightNullable, lateral);
            if (newRight != right)
            {
        ...
        case LATERAL:
            return registerFrom(parentScope, usingScope, register, ((SqlCall) node).operand(0), enclosingNode, alias,
                extendList, forceNullable, true);

        case COLLECTION_TABLE:
            call = (SqlCall) node;
            operand = call.operand(0);
            newOperand = registerFrom(parentScope, usingScope, register, operand, enclosingNode, alias, extendList,
                forceNullable, lateral);
        ...
        case UNNEST:
            if (!lateral)
            {
                return registerFrom(parentScope, usingScope, register, node, enclosingNode, alias, extendList,
                    forceNullable, true);
            }
            // fall through
        case SELECT:
        ...
        case OVER:
            ...
            operand = call.operand(0);
            newOperand = registerFrom(parentScope, overScope, true, operand, enclosingNode, alias, extendList,
                forceNullable, lateral);
            if (newOperand != operand)
            {
                call.setOperand(0, newOperand);
        ...
        case TABLE_REF:
            call = (SqlCall) node;
            registerFrom(parentScope, usingScope, register, call.operand(0), enclosingNode, alias, extendList,
                forceNullable, lateral);
            if (extendList != null && !extendList.isEmpty())
            {
                return enclosingNode;
        ...
        case EXTEND:
            ...
            return registerFrom(parentScope, usingScope, true, extend.getOperandList().get(0), extend, alias,
                (SqlNodeList) extend.getOperandList().get(1), forceNullable, lateral);
...
/**
 * Creates a namespace for a set operation (<code>UNION</code>, <code> INTERSECT</code>, or <code>EXCEPT</code>).
 ...
 */
protected void registerQuery(SqlValidatorScope parentScope, @Nullable SqlValidatorScope usingScope, SqlNode node,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable)
{
 ...
 */
private void registerQuery(SqlValidatorScope parentScope, @Nullable SqlValidatorScope usingScope, SqlNode node,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable, boolean checkUpdate)
{
    requireNonNull(node, "node");
    ...
    {
        case UDF_CREATE:
            final SqlNode functionNode = node;
            final SqlValidatorNamespace cfgNamespace =
                CalciteForkSettings.createFunctionNamespace(this, functionNode);
            registerNamespace(usingScope, alias, cfgNamespace, forceNullable);
            final SqlValidatorScope scope =
                CalciteForkSettings.createFunctionScope(parentScope, functionNode);
            scopes.put(functionNode, scope);
            registerOperandSubQueries(scope, (SqlCall) functionNode,
                CalciteForkSettings.functionBodyOperand(functionNode));
            break;
        case SELECT:
            ...
            if (select.getGroup() != null)
            {
                ...
                for (SqlNode groupByNode : groupByList)
                {
                    if (groupByNode instanceof SqlIdentifier
                        && ((SqlIdentifier) groupByNode).isStar())
                    {
                    ...
                    {
                        if (CalciteForkSettings.databricks() && groupByNode instanceof SqlCall)
                        {
                            ...
                            if (!selectList.contains(groupByNode))
                            {
                                ...
                                {
                                    if (selectItem instanceof SqlCall
                                        && ((SqlCall) selectItem).getOperator().getKind() == SqlKind.AS
                                        && ((SqlCall) selectItem).operand(0)
        ...
        case LAMBDA:
            call = (SqlCall) node;
            E6SqlLambdaScope lambdaScope = new E6SqlLambdaScope(parentScope, (SqlLambda) call);
            scopes.put(call, lambdaScope);
            final LambdaNamespace lambdaNamespace = new LambdaNamespace(this, (SqlLambda) call, node);
        ...
        case DELETE:
            ...
            registerQuery(parentScope, usingScope, SqlNonNullableAccessors.getSourceSelect(deleteCall), enclosingNode,
                null, false);
            break;
        ...
        case UPDATE:
            ...
            registerQuery(parentScope, usingScope, SqlNonNullableAccessors.getSourceSelect(updateCall), enclosingNode,
                null, false);
            break;
        ...
        case MERGE:
            ...
            registerQuery(parentScope, usingScope, SqlNonNullableAccessors.getSourceSelect(mergeCall), enclosingNode,
                null, false);
            ...
            {
                registerQuery(getScope(SqlNonNullableAccessors.getSourceSelect(mergeCall), Clause.WHERE), null,
                    mergeUpdateCall, enclosingNode, null, false, false);
            }
    ...
    List<SqlValidatorNamespace> fromNamespaceList, List<String> fromScopeChildNames)
{
    if (sqlNode != null && aggOrOverOrGroupFinder.findAgg(sqlNode) == null)
    {
        ...
        if (sqlNode instanceof SqlIdentifier
            && ((SqlIdentifier) sqlNode).isSimple() && CalciteForkSettings.databricks()
            && fromNamespaceList.size() > 1)
        {
            ...
            try
            {
                ...
                for (SqlValidatorNamespace fromNamespace : fromNamespaceList)
                {
                    ...
                    {
                        newGroupByList.add(
                            new SqlIdentifier(ImmutableList.of(fromScopeChildNames.get(fromScopeChildIndex), node.getSimple()),
...
private void registerSetop(SqlValidatorScope parentScope, @Nullable SqlValidatorScope usingScope, SqlNode node,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable)
{
    SqlCall call = (SqlCall) node;
    ...
    if (enclosingNode.getKind() == SqlKind.WITH_ITEM)
    {
        ...
        recursiveScope = scope != null && scope.recursiveScope != null ? requireNonNull(scope.recursiveScope)
                                                                       : parentScope;
    }
...
private void registerWith(SqlValidatorScope parentScope, @Nullable SqlValidatorScope usingScope, SqlWith with,
    SqlNode enclosingNode, @Nullable String alias, boolean forceNullable, boolean checkUpdate)
{
    ...
    for (SqlNode withItem_ : with.withList)
    {
        ...
        // changes by E6Data
        // replaced WithScope with E6WithScope for cte name case sensitivity
        final SqlValidatorScope withScope = new E6WithScope(scope, withItem,
...
{
    AggFinder nestedAggFinder = new AggFinder(opTab, false, false, false, aggFinder, catalogReader.nameMatcher());
...
/**
 * Returns the parse tree node (GROUP BY, HAVING, or an aggregate function call) that causes {@code select} to be an
 * aggregate query, or null if it is not an aggregate query.
 *
...
private void registerSubQueries(SqlValidatorScope parentScope, @Nullable SqlNode node)
{
    ...
    if (node.getKind().belongsTo(SqlKind.QUERY) || node.getKind() == SqlKind.LAMBDA
    ...
    else if (node instanceof SqlNodeList)
    {
        ...
        for (int i = 0, count = list.size(); i < count; i++)
        {
            ...
            {
                isSubQuery = true; // E6Data change for subquery
...
/**
 * Registers any sub-queries inside a given call operand, and converts the operand to a scalar sub-query if the operator
 * requires it.
 *
...
public void validateIntervalQualifier(SqlIntervalQualifier qualifier)
{
    ...
    if (startPrecisionOutOfRange)
    {
        throw newValidationError(qualifier,
            RESOURCE.intervalStartPrecisionOutOfRange(startPrecision, "INTERVAL " + qualifier));
    }
    else if (fractionalSecondPrecisionOutOfRange)
    {
        throw newValidationError(qualifier,
            RESOURCE.intervalFractionalSecondPrecisionOutOfRange(fracPrecision, "INTERVAL " + qualifier));
    }
}
...
/**
 * Validates the FROM clause of a query, or (recursively) a child node of the FROM clause: AS, OVER, JOIN, VALUES, or
 * sub-query.
 *
 * @param node Node in FROM clause, typically a table or derived table
 * @param targetRowType Desired row type of this expression, or {@link #unknownType} if not fussy. Must not be
 *     null.
 * @param scope Scope
 */
    ...
    if (call.getOperator() instanceof SqlTableFunction)
    {
        ...
        for (int idx = 0; idx < call.operandCount(); idx++)
        {
            ...
            if (tableCharacteristic != null)
            {
                ...
                {
                    throw newValidationError(call, RESOURCE.multipleRowSemanticsTables(call.getOperator().getName()));
                }
...
private void throwInvalidRowSemanticsTable(SqlCall call, int idx, SqlCall table)
{
    ...
    {
        throw newValidationError(call, RESOURCE.invalidPartitionKeys(idx, call.getOperator().getName()));
    }
    SqlNodeList orderList = table.operand(2);
    if (!orderList.isEmpty())
    {
        throw newValidationError(call, RESOURCE.invalidOrderBy(idx, call.getOperator().getName()));
    }
}
...
{
    // E6Data We infer the unknown type inside the call, for example this calls the TYPE_INFERENCE inside functions
    inferUnknownTypes(unknownType, scope, call);
    for (int i = 0; i < call.operandCount(); i++)
    {
...
protected void validateJoin(SqlJoin join, SqlValidatorScope scope)
{
    ...
    switch (conditionType)
    {
        ...
        case ON:
            ...
            break;
        case USING:
            @SuppressWarnings({"rawtypes", "unchecked"}) List<SqlIdentifier> list = (List) getCondition(join);
    ...
    if (natural)
    {
        ...
        {
            final SqlIdentifier id = new SqlIdentifier(name, join.isNaturalNode().getParserPosition());
            validateCommonJoinColumn(id, left, right, scope, natural);
        }
    }
    ...
    switch (joinType)
    {
        ...
        case ASOF:
        {
            ...
            CompareFromBothSides validateCompare = new CompareFromBothSides(joinScope, catalogReader,
            ...
            validateCompare = new CompareFromBothSides(joinScope, catalogReader, RESOURCE.asofMatchMustBeComparison());
...
/**
 * Shuttle which determines whether all SqlCalls that are comparisons are comparing columns from both namespaces. The
 * shuttle will throw an exception if that happens. If it returns all SqlCalls have the expected shape.
 */
private class CompareFromBothSides extends SqlShuttle
{
    ...
    private CompareFromBothSides(SqlValidatorScope scope, SqlValidatorCatalogReader catalogReader,
...
/**
 * Shuttle which determines whether an expression is a simple conjunction of equalities. Each equality may involve a
 * cast
 */
 ...
 */
private void validateCommonJoinColumn(SqlIdentifier id, SqlNode left, SqlNode right, SqlValidatorScope scope,
    boolean natural)
{
    if (id.names.size() != 1)
    {
    ...
    final RelDataType leftColType = natural ? checkAndDeriveDataType(id, left)
    ...
    {
        throw newValidationError(id, RESOURCE.naturalOrUsingColumnNotCompatible(id.getSimple(), leftColType.toString(),
...
private RelDataType checkAndDeriveDataType(SqlIdentifier id, SqlNode node)
{
    ...
    RelDataType colType = requireNonNull(nameMatcher.field(rowType, name),
        () -> "unable to find left field " + name + " in " + rowType).getType();
    return colType;
}

/**
 * Validates a column in a USING clause, or an inferred join key in a NATURAL join, in the left or right input to the
 * join.
 */
private RelDataType validateCommonInputJoinColumn(SqlIdentifier id, SqlNode leftOrRight, SqlValidatorScope scope,
    boolean natural)
{
    checkArgument(id.names.size() == 1);
...
protected void validateSelect(SqlSelect select, RelDataType targetRowType)
{
    ...
    if (duplicateAliasOrdinal >= 0)
    {
        ...
        throw newValidationError(
            requireNonNull(child.namespace.getEnclosingNode(), () -> "enclosingNode of namespace of " + child.name),
...
/**
 * For each identifier in an expression, resolves it to a qualified name and calls the provided action.
 */
private static void forEachQualified(SqlNode node, SqlValidatorScope scope, Consumer<SqlQualified> consumer)
{
...
/**
 * Removes all entries from {@code qualifieds} and {@code remnantMustFilterFields} if {@code node} is a bypassField.
 */
private static void purgeForBypassFields(SqlNode node, SqlValidatorScope scope, Set<SqlQualified> qualifieds,
    Set<SqlQualified> bypassQualifieds, Set<SqlQualified> remnantMustFilterFields)
{
    node.accept(new SqlBasicVisitor<Void>()
    {
        @Override
...
private static void toQualifieds(ImmutableBitSet fields, Set<SqlQualified> qualifiedSet, SelectScope fromScope,
    ScopeChild child, List<String> fieldNames)
{
    fields.forEachInt(i -> qualifiedSet.add(SqlQualified.create(fromScope, 1, child.namespace,
        new SqlIdentifier(ImmutableList.of(child.name, fieldNames.get(i)), SqlParserPos.ZERO))));
}
...
/**
 * If the {@code node} is a DOT call, returns its first operand. Recurse, if the first operand is another DOT call.
 *
...
private void checkRollUp(@Nullable SqlNode grandParent, @Nullable SqlNode parent, @Nullable SqlNode current,
    SqlValidatorScope scope, @Nullable String contextClause)
{
    ...
    if (current instanceof SqlCall && !(current instanceof SqlSelect))
    {
        ...
        else if (stripDot.getKind() == SqlKind.CONVERT || stripDot.getKind() == SqlKind.TRANSLATE
    ...
    else if (current instanceof SqlIdentifier)
    {
        ...
        {
            if (!isAggregation(requireNonNull(parent, "parent").getKind()) || !isRolledUpColumnAllowedInAgg(id, scope,
                (SqlCall) parent, grandParent))
            {
                String context = contextClause != null ? contextClause : parent.getKind().toString();
                throw newValidationError(id, RESOURCE.rolledUpNotAllowed(SqlValidatorUtil.alias(id, 0), context));
            }
        }
    }
}

private void checkRollUp(@Nullable SqlNode grandParent, SqlNode parent, @Nullable SqlNode current,
...
/**
 * Validates that a query can deliver the modality it promises. Only called on the top-most SELECT or set operator in
 * the tree.
 */
                ...
                for (ScopeChild child : scope.children)
                {
                    if (!child.namespace.supportsModality(modality))
                    {
                        if (fail)
                        {
                            ...
                            throw newValidationError(node, Static.RESOURCE.cannotConvertToStream(child.name));
                        }
                        else
                        {
                            return false;
        ...
        default:
            for (ScopeChild child : scope.children)
            {
                if (!child.namespace.supportsModality(modality))
                {
                    ...
                    {
                        SqlNode node = SqlNonNullableAccessors.getNode(child);
                        throw newValidationError(node, Static.RESOURCE.cannotConvertToRelation(child.name));
                    }
                    else
                    {
                        return false;
...
protected void validateWindowClause(SqlSelect select)
{
    ...
    {
        SqlIdentifier declName = requireNonNull(window.getDeclName(), () -> "window.getDeclName() for " + window);
...
protected void validateMustFilterRequirements(SqlSelect select, SelectNamespace ns)
{
    ...
    if (select.getFrom() != null)
    {
        ...
        for (ScopeChild child : fromScope.children)
        {
            ...
            toQualifieds(filterRequirement.filterFields, qualifieds, fromScope, child, fieldNames);
            toQualifieds(filterRequirement.bypassFields, bypassQualifieds, fromScope, child, fieldNames);
        ...
        if (!qualifieds.isEmpty() || !bypassQualifieds.isEmpty())
        {
            if (select.getWhere() != null)
            {
                ...
                purgeForBypassFields(select.getWhere(), getWhereScope(select), qualifieds, bypassQualifieds,
                    remnantQualifieds);
            }
            if (select.getHaving() != null)
            {
                forEachQualified(select.getHaving(), getHavingScope(select), qualifieds::remove);
                purgeForBypassFields(select.getHaving(), getHavingScope(select), qualifieds, bypassQualifieds,
                    remnantQualifieds);
            }
            ...
            final List<SqlNode> expandedSelectItems = requireNonNull(fromScope.getExpandedSelectList(),
                "expandedSelectList");
            ...
            {
                throw newValidationError(select, RESOURCE.mustFilterFieldsMissing(qualifieds.stream()
                    .map(q -> q.suffix().get(0))
                    .collect(Collectors.toCollection(TreeSet::new))
                    .toString()));
            }
            ...
            ImmutableSet<SqlQualified> remnantMustFilterFields = Stream.of(remnantQualifieds, qualifieds)
                .flatMap(Set::stream)
                .collect(ImmutableSet.toImmutableSet());
            ns.filterRequirement = new FilterRequirement(ImmutableBitSet.fromBitSet(mustFilterFields),
                ImmutableBitSet.fromBitSet(mustFilterBypassFields), remnantMustFilterFields);
        }
    }
}
...
public void validateWithItem(SqlWithItem withItem)
{
    ...
    else
    {
        ...
        // e6 change - by pass duplicate check if flag enabled for kantar use case
        // this is wrong and can break things in calcite
        if (!CalciteForkSettings.allowDuplicateAliasInProjection() && i >= 0)
        {
...
private void validateGroupItem(SqlValidatorScope groupScope, @Nullable AggregatingSelectScope aggregatingScope,
    SqlNode groupItem)
{
...
private void validateGroupingSets(SqlValidatorScope groupScope, @Nullable AggregatingSelectScope aggregatingScope,
    SqlCall groupItem)
{
...
private static boolean isReturnBooleanType(RelDataType relDataType)
{
    if (relDataType instanceof RelRecordType)
    {
        ...
        checkState(recordType.getFieldList().size() == 1, "sub-query as condition must return only one column");
...
protected RelDataType validateSelectList(final SqlNodeList selectItems, SqlSelect select, RelDataType targetRowType)
{
    ...
    for (SqlNode selectItem : selectItems)
    {
        ...
        else
        {
            ...
            final RelDataType fieldType = targetRowType.isStruct() && targetRowType.getFieldCount() > fieldIdx
                                          ? targetRowType.getFieldList().get(fieldIdx).getType() : unknownType;
...
private void validateExpr(SqlNode expr, SqlValidatorScope scope)
{
    ...
    if (!config.nakedMeasuresInNonAggregateQuery() && !(scope instanceof AggregatingScope) && scope.isMeasureRef(expr))
    {
...
/**
 * Processes SubQuery found in Select list. Checks that is actually Scalar sub-query and makes proper entries in each of
 * the 3 lists used to create the final rowType entry.
 *
 ...
 * @return Rowtype
 */
protected RelDataType createTargetRowType(SqlValidatorTable table, @Nullable SqlNodeList targetColumnList,
    boolean append, @Nullable SqlIdentifier targetTableAlias)
{
...
public void validateInsert(SqlInsert insert)
{
    ...
    final RelOptTable relOptTable = SqlValidatorUtil.getRelOptTable(targetNamespace,
        catalogReader.unwrap(Prepare.CatalogReader.class), null, null);
    final SqlValidatorTable table = relOptTable == null ? getTable(targetNamespace)
    ...
    final RelDataType targetRowType = createTargetRowType(table, insert.getTargetColumnList(), false, null);
    ...
    final RelDataType realTargetRowType = typeFactory.createStructType(logicalTargetRowType.getFieldList()
        .stream()
        .filter(f -> strategies.get(f.getIndex()).canInsertInto())
        .collect(Collectors.toList()));

    final RelDataType targetRowTypeToValidate =
        logicalSourceRowType.getFieldCount() == logicalTargetRowType.getFieldCount() ? logicalTargetRowType
                                                                                     : realTargetRowType;

    checkFieldCount(insert.getTargetTable(), table, strategies, targetRowTypeToValidate, realTargetRowType, source,
...
private void checkConstraint(SqlValidatorTable validatorTable, SqlNode source, RelDataType targetRowType)
{
    ...
    if (modifiableViewTable != null && source instanceof SqlCall)
    {
        ...
        @SuppressWarnings("RedundantCast") final ImmutableBitSet targetColumns = ImmutableBitSet.of(
            (Iterable<Integer>) tableIndexToTargetField.keySet());
        @SuppressWarnings("RedundantCast") final ImmutableBitSet constrainedColumns = ImmutableBitSet.of(
            (Iterable<Integer>) projectMap.keySet());
        @SuppressWarnings("assignment.type.incompatible") List<@KeyFor({"tableIndexToTargetField",
            "projectMap"}) Integer> constrainedTargetColumns = targetColumns.intersect(constrainedColumns).asList();
        ...
        for (final int colIndex : constrainedTargetColumns)
        {
            ...
            final RelDataTypeField targetField = requireNonNull(tableIndexToTargetField.get(colIndex));
 ...
 * @param logicalTargetRowType logical target row type, contains only target columns if they are specified or if the
 *     sql dialect allows subset insert, make a subset of fields(start from the left first field) whose length is equals
 *     with the source row type fields number
 */
private void checkFieldCount(SqlNode node, SqlValidatorTable table, List<ColumnStrategy> strategies,
    RelDataType targetRowTypeToValidate, RelDataType realTargetRowType, SqlNode source,
 ...
 * check if the source fields count equals with the real target table fields count, see how #checkFieldCount was used.
 *
 * @param sourceScope Scope of query source which is used to infer node type
 * @param table Target table
 * @param sourceRowType Source row type
 * @param targetRowType Target row type, it should either contain all the virtual columns (can not insert into) or
 *     exclude all the virtual columns
 * @param query The query
 */
protected void checkTypeAssignment(@Nullable SqlValidatorScope sourceScope, SqlValidatorTable table,
    RelDataType sourceRowType, RelDataType targetRowType, final SqlNode query)
{
    ...
    for (int i = 0; i < sourceCount; ++i)
    {
        ...
        if (!SqlTypeUtil.canAssignFrom(targetType, sourceType))
        {
            ...
            throw newValidationError(node, RESOURCE.typeNotAssignable(targetFields.get(i).getName(), targetTypeString,
...
private static SqlNode getNthExpr(SqlNode query, int ordinal, int sourceCount)
{
    ...
    else if (query instanceof SqlUpdate)
    {
        ...
        {
            return getNthExpr(SqlNonNullableAccessors.getSourceSelect(update), ordinal, sourceCount);
        }
    }
...
public void validateUpdate(SqlUpdate call)
{
    ...
    validateNamespace(targetNamespace, unknownType);
    final RelOptTable relOptTable = SqlValidatorUtil.getRelOptTable(targetNamespace,
        castNonNull(catalogReader.unwrap(Prepare.CatalogReader.class)), null, null);
    final SqlValidatorTable table = relOptTable == null ? getTable(targetNamespace)
                                                        : relOptTable.unwrapOrThrow(SqlValidatorTable.class);

    final RelDataType targetRowType = createTargetRowType(table, call.getTargetColumnList(), true, call.getAlias());
...
public void validateMerge(SqlMerge call)
{
    ...
    if (updateCall != null)
    {
        ...
        targetRowType = createTargetRowType(table, updateCall.getTargetColumnList(), true, call.getAlias());
    }
    SqlInsert insertCall = call.getInsertCall();
    if (insertCall != null)
    {
        requireNonNull(table, () -> "ns.getTable() for " + targetNamespace);
        targetRowType = createTargetRowType(table, insertCall.getTargetColumnList(), false, null);
    }
    ...
    if (insertCallAfterValidate != null)
    {
        ...
        if (insertCallAfterValidate.getSource() instanceof SqlSelect)
        {
            ...
            for (int i = 0; i < sourceSelectList.size(); i++)
            {
                ...
                {
                    throw newValidationError(selectItem, RESOURCE.columnNotNullable(targetField.getName()));
                }
            }
        }
    }
...
private void validateSnapshot(SqlNode node, @Nullable SqlValidatorScope scope, SqlValidatorNamespace ns)
{
    ...
    {
        E6SqlSnapshot snapshot = (E6SqlSnapshot) node;
        SqlValidatorTable validatorTable = getTable(ns);
        Table table = validatorTable.table();
        if (CalciteForkSettings.supportsTimeTravel(table))
        {
            snapshot.validate(scope);
        }
        else
        {
            List<String> qualifiedName = validatorTable.getQualifiedName();
            String tableName = qualifiedName.get(qualifiedName.size() - 1);
            throw CalciteForkSettings.timeTravelNotSupportedException(snapshot.getTableRef(),
                tableName, CalciteForkSettings.tableTypeName(table));
        }
    }
}
...
protected void validateValues(SqlCall node, RelDataType targetRowType, final SqlValidatorScope scope)
{
    ...
    for (SqlNode operand : operands)
    {
        ...
        if (this.config.conformance().isInsertSubsetColumnsAllowed() && targetRowType.isStruct()
...
/**
 * Throws a validator exception with access to the validator context. The exception is determined when an instance is
 * created.
 */
...
/**
 * Throws a validator exception with access to the validator context. The exception is determined when the function is
 * applied.
 */
class ValidationErrorFunction implements
...
{
    final E6SqlLambdaScope scope = (E6SqlLambdaScope) scopes.get(lambdaExpr);
    requireNonNull(scope, "scope");
...
public void validateMatchRecognize(SqlCall call)
{
    ...
    for (SqlNode node : matchRecognize.getOrderList())
    {
        ...
        {
            identifier = requireNonNull((SqlIdentifier) node,
                () -> "order by field is null. All fields: " + matchRecognize.getOrderList());
        }
    ...
    if (interval != null)
    {
        ...
        if (((SqlIntervalLiteral) interval).signum() < 0)
        {
            ...
            throw newValidationError(interval,
                RESOURCE.intervalMustBeNonNegative(intervalValue != null ? intervalValue : interval.toString()));
        }
    ...
    PairList<String, RelDataType> measureColumns = validateMeasure(matchRecognize, scope, allRows);
...
private PairList<String, RelDataType> validateMeasure(SqlMatchRecognize mr, MatchRecognizeScope scope, boolean allRows)
{
    ...
    for (SqlNode measure : measures)
    {
        ...
        // added by E6data for lateral alias expansion
        SqlNode expand = expandMeasureAliasMatchRecognize(measure, scope, mr);
        ...
        fields.add(alias, type);
        sqlNodes.add(
            SqlStdOperatorTable.AS.createCall(SqlParserPos.ZERO, expand, new SqlIdentifier(alias, SqlParserPos.ZERO)));
    }
...
public void validatePivot(SqlPivot pivot) {
    ...
    // condition added by e6data to populate pivotRenamingColumns
    SqlNode enclosingPivotNode = namespaces.get(pivot).getEnclosingNode();
    if (!(enclosingPivotNode instanceof SqlPivot) && enclosingPivotNode instanceof SqlCall
        && ((SqlCall) enclosingPivotNode).getOperator().getKind() == SqlKind.AS
        && ((SqlCall) enclosingPivotNode).getOperandList().size() > 2)
    {
        SqlCall call = (SqlCall) enclosingPivotNode;
        for (int i = 2; i < call.getOperandList().size(); i++)
        {
            if (call.getOperandList().get(i) instanceof SqlIdentifier)
            {
                SqlIdentifier identifier = (SqlIdentifier) call.getOperandList().get(i);
                pivotRenamingColumns.put(identifier.getSimple(), null);
            }
        }
    }
...
public void validateUnpivot(SqlUnpivot unpivot)
{
    ...
    unpivot.forEachNameValues((nodeList, valueList) ->
    {
        ...
        {
            throw newValidationError(nodeList, RESOURCE.unpivotValueArityMismatch(nodeList.size(), measureCount));
        }
        ...
        {
            throw newValidationError(valueList, RESOURCE.unpivotValueArityMismatch(valueList.size(), axisCount));
        }
    ...
    forEach(unpivot.measureList, (measure, i) ->
    {
        ...
        if (type0 == null)
        {
            throw newValidationError(nodes.get(0), RESOURCE.unpivotCannotDeriveMeasureType(measureName));
        }
    ...
    forEach(unpivot.axisList, (axis, i) ->
    {
        ...
        unpivot.forEachNameValues((aliasList, valueList) -> types.add(
            valueList == null ? typeFactory.createSqlType(SqlTypeName.VARCHAR,
                SqlUnpivot.aliasValue(aliasList).length()) : deriveType(scope, valueList.get(i))));
        final RelDataType type = typeFactory.leastRestrictive(types);
        if (type == null)
        {
            throw newValidationError(axis, RESOURCE.unpivotCannotDeriveAxisType(axisName));
        }
 ...
 * {@code PREV(B.price)} to {@code LAST(B.price, 0)}.
 */
...
@Override
public void validateAggregateParams(SqlCall aggCall, @Nullable SqlNode filter, @Nullable SqlNodeList distinctList,
    @Nullable SqlNodeList orderList, SqlValidatorScope scope)
{
    ...
    if (op.isPercentile())
    {
        ...
        {
            case 1:
                // The window form (WITHIN GROUP + OVER) is rewritten to use
                // FORBIDDEN group order with 1 operand; skip WITHIN GROUP
                // validation since the order was moved to the window clause.
                if (op.requiresGroupOrder() == Optionality.MANDATORY)
                {
                    ...
                    if (family == null || family.allowableDifferenceTypes().isEmpty())
                    {
                        throw newValidationError(orderList,
                            RESOURCE.unsupportedTypeInOrderBy(type.getSqlTypeName().getName(), op.getName()));
                    }
                }
                break;
            case 2:
...
/**
 * Validates that a particular feature is enabled. By default, all features are enabled; subclasses may override this
 * method to be more discriminating.
 *
...
public SqlLiteral resolveLiteral(SqlLiteral literal)
{
    switch (literal.getTypeName())
    {
        case UNKNOWN:
            ...
            final SqlIdentifier identifier = new SqlIdentifier(unknownLiteral.tag, SqlParserPos.ZERO);
...
// new method added by e6data to expand select expression if pivot is present
public SqlNode expandPivotSelectExpr(SqlNode expr, SelectScope scope, SqlSelect select, PivotNamespace pivotNamespace)
{
    final PivotSelectExpander expander = new PivotSelectExpander(this, scope, select, pivotNamespace,
        pivotRenamingColumns.keySet().stream().collect(Collectors.toList()));
    SqlNode newExpr = expander.go(expr);

    String alias = expander.getAlias();
    RelDataType datatype = expander.getDatatype();
    pivotRenamingColumns.replace(alias, datatype);
...
// added by E6Data for expanding columns inside match recognize measure with lateral alias
public SqlNode expandMeasureAliasMatchRecognize(SqlNode expr, SqlValidatorScope scope, SqlMatchRecognize matchRecognize)
{
    final Expander expander = new MatchRecognizeMeasureAliasExpander(this, scope, matchRecognize);
    SqlNode newExpr = expander.go(expr);
    if (expr != newExpr)
    {
        setOriginal(newExpr, expr);
...
private @Nullable List<String> getFieldOrigin(SqlNode sqlQuery, int i)
{
    if (sqlQuery instanceof SqlSelect)
    {
        ...
        if (selectItem instanceof SqlIdentifier)
        {
            ...
            SqlValidatorNamespace namespace = requireNonNull(qualified.namespace, () -> "namespace for " + qualified);
            if (namespace.isWrapperFor(AliasNamespace.class))
            {
                AliasNamespace aliasNs = namespace.unwrap(AliasNamespace.class);
                SqlNode aliased = requireNonNull(aliasNs.getNode(), () -> "sqlNode for aliasNs " + aliasNs);
...
// new method by e6data
public List<String> getPivotRenamingColumns()
{
    return pivotRenamingColumns.keySet().stream().collect(Collectors.toList());
}

// new method by E6data
public boolean hasPivotWithStar()
{
    return hasPivotWithStar;
}
...
private class DeriveTypeVisitor implements SqlVisitor<RelDataType>
{
    ...
    {
        RelDataType type = resolveLiteral(literal).createSqlType(typeFactory);
        // added by E6Data
        // if literal is a character, then cast it to varchar
        // because we are casting every literal to varchar in conversion
        // see convertExtendedExpression() method in E6SqlToRelConverter.java
        if (CalciteForkSettings.castCharLiteralToVarchar()
            && type.getSqlTypeName().equals(SqlTypeName.CHAR))
        {
            RelDataType newType = typeFactory.createSqlType(SqlTypeName.VARCHAR, type.getPrecision());
            newType = typeFactory.createTypeWithNullability(newType, type.isNullable());
            if (type.getCharset() != null && type.getCollation() != null)
            {
                newType = typeFactory.createTypeWithCharsetAndCollation(newType, type.getCharset(),
                    type.getCollation());
            }
            return newType;
        }
    ...
    public RelDataType visit(SqlIdentifier id)
    {
        ...
        // added condition by E6data to get relDatatype for pivot renamed column
        RelDataType type = null;
        String columnName = id.names.get(id.names.size() - 1);
        if (!pivotRenamingColumns.isEmpty() && pivotRenamingColumns.containsKey(columnName))
        {
            RelDataType pivotColumnType = pivotRenamingColumns.get(columnName);
            if (pivotColumnType != null)
            {
                return pivotColumnType;
            }

            throw newValidationError(id, RESOURCE.unknownIdentifier(columnName));
        }
        else if (!(scope instanceof EmptyScope))
        {
...
private static class Expander extends SqlScopedShuttle
{
    ...
    {
        if (DynamicRecordType.isDynamicStarColName(Util.last(fqId.names)) && !DynamicRecordType.isDynamicStarColName(
            Util.last(id.names)))
        {
            ...
            return new SqlBasicCall(SqlStdOperatorTable.ITEM,
                ImmutableList.of(fqId, SqlLiteral.createCharString(Util.last(id.names), id.getParserPosition())),
...
/**
 * Shuttle which walks over an expression in the ORDER BY clause, replacing usages of aliases with the underlying
 * expression.
 */
class OrderExpressionExpander extends SqlScopedShuttle
{
    ...
    public SqlNode go()
    {
        return requireNonNull(root.accept(this), () -> "OrderExpressionExpander returned null for " + root);
    }
    ...
    private SqlNode nthSelectItem(int ordinal, final SqlParserPos pos)
    {
        ...
        SqlNodeList expandedSelectList = expandStar(SqlNonNullableAccessors.getSelectList(select), select, false);
        SqlNode expr = expandedSelectList.get(ordinal);
        expr = stripAs(expr);
        // added by E6data
        // skip fully qualify if pivot renamed column is present
        if (expr instanceof SqlIdentifier
            && !pivotRenamingColumns.containsKey(((SqlIdentifier) expr).names.get(((SqlIdentifier) expr).names.size() - 1)))
        {
...
/**
 * Expander added by E6data to expand select statement having pivot renaming columns
 */
static class PivotSelectExpander extends Expander
{

    private final SqlSelect select;
    private final SelectScope scope;
    private ScopeChild m_scopeChild = null;
    private final PivotNamespace m_pivotNamespace;
    private final List<String> pivotRenamingColumns;
    private String pivotRenameColumnAlias = null;
    private RelDataType pivotRenameColumnDatatype = null;
    private boolean isPivotRenamingColumn = false;
    private String pivotOriginalColumnName = null;

    PivotSelectExpander(SqlValidatorImpl validator, SelectScope scope, SqlSelect select, PivotNamespace namespace,
        List<String> pivotRenamingColumns)
    {
        super(validator, scope);
        this.select = select;
        this.scope = scope;
        m_pivotNamespace = namespace;
        this.pivotRenamingColumns = pivotRenamingColumns;
    }

    @Override
    public @Nullable SqlNode visit(SqlIdentifier id)
    {
        SqlNode expanded = null;
        List<RelDataTypeField> fieldList = m_pivotNamespace.getRowType().getFieldList();

        String columnName = id.names.get(id.names.size() - 1);
        for (int i = 0; i < pivotRenamingColumns.size(); i++)
        {
            if (pivotRenamingColumns.get(i).equals(columnName))
            {
                expanded = new SqlBasicCall(SqlStdOperatorTable.AS,
                    ImmutableList.of(new SqlIdentifier(fieldList.get(i).getName(), id.getParserPosition()),
                        new SqlIdentifier(ImmutableList.of(columnName), id.getParserPosition())), id.getParserPosition());
                pivotRenameColumnAlias = columnName;
                pivotOriginalColumnName = fieldList.get(i).getName();
                pivotRenameColumnDatatype = fieldList.get(i).getType();
                isPivotRenamingColumn = true;
                break;
            }
        }
    ...
    {
        return pivotRenameColumnAlias;
    }

    public RelDataType getDatatype()
    {
        return pivotRenameColumnDatatype;
    }

    public boolean isPivotRenamingColumn()
    {
        return isPivotRenamingColumn;
    }

    public String getPivotOriginalColumnName()
    {
        return pivotOriginalColumnName;
    }
...
static class ExtendedExpander extends Expander
{
    ...
    public @Nullable SqlNode visit(SqlIdentifier id)
    {
        ...
        for (SqlNode s : SqlNonNullableAccessors.getSelectList(select))
        {
            ...
            if (alias != null && nameMatcher.matches(alias, name))
            {
                if (CalciteForkSettings.databricks() && clause != Clause.HAVING)
                {
    ...
    /**
     * Add all possible expandable 'group by' expression to set, which is used to check whether expr could be expanded
     * as alias or ordinal.
     */
 ...
 */
static class MatchRecognizeMeasureAliasExpander extends Expander
{

    SqlMatchRecognize m_sqlMatchRecognize;

    MatchRecognizeMeasureAliasExpander(SqlValidatorImpl validator, SqlValidatorScope scope,
        SqlMatchRecognize matchRecognize)
    {
        super(validator, scope);
        this.m_sqlMatchRecognize = matchRecognize;
    }
    ...
    public @Nullable SqlNode visit(SqlIdentifier id)
    {
        if (id.isSimple())
        {
            ...
            catch (Exception e)
            {
                ...
                final SqlNameMatcher nameMatcher = validator.catalogReader.nameMatcher();
                int n = 0;
                for (SqlNode measure : m_sqlMatchRecognize.getMeasureList())
                {
                ...
                final Expander expander = new MatchRecognizeMeasureAliasExpander(validator, getScope(),
                    m_sqlMatchRecognize);
                return expr.accept(expander);
            }
        }
...
protected static class FunctionParamInfo
{
    ...
    /**
     * Maps a cursor (based on its position relative to other cursor parameters within a function call) to the SELECT
     * associated with the cursor.
     */
    public final Map<Integer, SqlSelect> cursorPosToSelectMap;

    /**
     * Maps a column list parameter to the parent cursor parameter it references. The parameters are id'd by their
     * names.
     */
...
private static class NavigationModifier extends SqlShuttle
{
    ...
    {
        return requireNonNull(node.accept(this), () -> "NavigationModifier returned for " + node);
    }
        ...
        if (isLogicalNavigation(kind) || isPhysicalNavigation(kind))
        {
            ...
            if (isPhysicalNavigation(kind))
            {
                ...
                if (isPhysicalNavigation(innerKind))
                {
                    ...
                    SqlOperator newOperator = innerKind == kind ? SqlStdOperatorTable.PLUS : SqlStdOperatorTable.MINUS;
 ...
 * implementation of {@link RexVisitor#visitPatternFieldRef(RexPatternFieldRef)} more unified. Otherwise, it's difficult
 * to implement this method. If it returns the specified field, then the navigation such as {@code PREV(A.price, 1)}
 * becomes impossible; if not, then comparisons such as {@code A.price > PREV(A.price, 1)} become meaningless.
 */
        ...
        switch (kind)
        {
            case PREV:
                ...
                if (operands.get(0) instanceof SqlIdentifier)
                {
                    ...
                    return name.equals(alpha) ? call : SqlStdOperatorTable.LAST.createCall(SqlParserPos.ZERO, operands);
                }
                break;
            default:
                break;
...
private class PatternValidator extends SqlBasicVisitor<@Nullable Set<String>>
{
    ...
    public Set<String> visit(SqlCall call)
    {
        ...
        for (SqlNode node : operands)
        {
            if (node != null)
            {
                vars.addAll(requireNonNull(
                    node.accept(new PatternValidator(isMeasure, firstLastCount, prevNextCount, aggregateCount)),
            ...
            case JOIN:
                ...
                if (names != null)
                {
                    for (String name : names)
                    {
                        ...
                        final boolean nullable = (f.getType().isNullable() || join.getJoinType().generatesNullsOnLeft())
                            && (f2.getType().isNullable() || join.getJoinType().generatesNullsOnRight());
        ...
        for (ImmutableIntList source : sources)
        {
            ...
            for (int p1 : Util.skip(source))
            {
                ...
                selectItem = SqlStdOperatorTable.AS.createCall(SqlParserPos.ZERO,
                    SqlStdOperatorTable.COALESCE.createCall(SqlParserPos.ZERO, maybeCast(selectItem, type, type2),
                        maybeCast(selectItem1, type1, type2)), new SqlIdentifier(name, SqlParserPos.ZERO));
