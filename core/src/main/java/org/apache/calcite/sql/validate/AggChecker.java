...
class AggChecker extends SqlBasicVisitor<Void> {
   ...
   * @param scope      Scope
   * @param extraExprs Expressions in GROUP BY (or SELECT DISTINCT) clause, that are therefore
   *     available
  ...
  @Override public Void visit(SqlIdentifier id) {
    ...
    if (!validator.config().nakedMeasuresInAggregateQuery() && isMeasureExp(id)) {
      SqlNode originalExpr = validator.getOriginal(id);
      throw validator.newValidationError(originalExpr, RESOURCE.measureIllegal());
    }
    ...
    throw validator.newValidationError(
        originalExpr,
        distinct ? RESOURCE.notSelectDistinctExpr(exprString) : RESOURCE.notGroupExpr(exprString));
  }

  @Override public Void visit(SqlCall call) {
    final SqlValidatorScope scope = requireNonNull(scopes.peek(), () -> "scope for " + call);
    ...
    if (call.getKind() == SqlKind.OVER) {
      ...
      if (scope instanceof AggregatingSelectScope) {
        ...
        SelectScope selectScope =
            requireNonNull(
                validator.getRawSelectScope(select), () -> "rawSelectScope for " + scope.getNode());
        List<SqlNode> selectList =
            requireNonNull(
                selectScope.getExpandedSelectList(), () -> "expandedSelectList for " + selectScope);
