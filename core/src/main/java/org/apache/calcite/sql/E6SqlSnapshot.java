...
public class E6SqlSnapshot extends SqlCall {
  ...
  public E6SqlSnapshot(
      SqlParserPos pos, SqlNode tableRef, SqlNode period, SqlCharStringLiteral type) {
    super(pos);
  ...
  @Override public void setOperand(int i, @Nullable SqlNode operand) {
    ...
    case OPERAND_PERIOD:
      ...
      break;
    case OPERAND_TYPE:
      type = (SqlCharStringLiteral) requireNonNull(operand, "operand");
      break;
    default:
  ...
  public void validate(SqlValidatorScope scope) throws CalciteContextException {
    ...
    case TYPE_VERSION:
      ...
      break;
    default:
      throw CalciteForkSettings.validationException(
            period, "Invalid type expected for snapshot, this shouldn't happen ever");
    }
    ...
    if (SqlTypeUtil.isNull(dataType) || SqlTypeUtil.containsNullable(dataType)) {
      throw CalciteForkSettings.validationException(
          period,
          "The time travel specification expects ''{0}'' type but is NULL",
          targetFamily,
          dataType);
    }
    ...
    if (!SqlTypeUtil.canCastFrom(targetType, dataType, true)) {
      throw CalciteForkSettings.validationException(
          period,
          "The time travel specification expects ''{0}'' type but is ''{1}''",
          targetFamily,
          dataType);
    }

    if (!E6TypeCoersionHelper.needToCast(
        scope, period, targetType, (AbstractTypeCoercion) typeCoercion)) {
      return;
    }

    E6TypeCoersionHelper.coerceOperandType(
        scope, this, OPERAND_PERIOD, targetType, (AbstractTypeCoercion) typeCoercion);
  }

  /** An operator describing a FOR SYSTEM_TIME specification. */
  public static class E6SqlSnapshotOperator extends SqlOperator {

    public static final E6SqlSnapshot.E6SqlSnapshotOperator INSTANCE =
        new E6SqlSnapshot.E6SqlSnapshotOperator();
