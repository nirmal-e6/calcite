...
/**
 * RexNodeAndFieldIndex has the same meaning as {@link RexInputRef}, they are both reference a field
 * of an input relational expression. The difference is that, RexNodeAndFieldIndex uses the input
 * index and the relative position of field in input rowType.
 *
 * <p>For example, if the inputs to a join are
 *
 * <ul>
 *   <li>Input #0: EMP(EMPNO, ENAME, DEPTNO) and
 *   <li>Input #1: DEPT(DEPTNO AS DEPTNO2, DNAME)
 * </ul>
 *
 * <p>then the fields are:
 *
 * <ul>
 *   <li>Node #0, Field #0: EMPNO
 *   <li>Node #0, Field #1: ENAME
 *   <li>Node #0, Field #2: DEPTNO (from EMP)
 *   <li>Node #1, Field #0: DEPTNO2 (from DEPT)
 *   <li>Node #1, Field #1: DNAME
 * </ul>
 *
 * <p>If in some cases, the inputs order of a relation is frequently adjusted and it is difficult to
 * maintain the correct RexInputRef, you can consider temporarily replacing RexInputRef with
