...
protected E6RelBuilder(@Nullable Context context, RelOptCluster cluster, @Nullable RelOptSchema relOptSchema)
{
    ...
    final RexExecutor executor = context.maybeUnwrap(RexExecutor.class)
...
/**
 * Derives the view expander {@link org.apache.calcite.plan.RelOptTable.ViewExpander} to be used for this RelBuilder.
 *
 * <p>The ViewExpander instance is used for expanding views in the default
 * table scan factory {@code RelFactories.TableScanFactoryImpl}. You can also define a new table scan factory in the
...
public static RelBuilder create(FrameworkConfig config)
{
    return Frameworks.withPrepare(config,
        (cluster, relOptSchema, rootSchema, statement) -> new E6RelBuilder(config.getContext(), cluster, relOptSchema));
}
...
/**
 * Creates a {@link RelBuilderFactory}, a partially-created RelBuilder. Just add a {@link RelOptCluster} and a
 * {@link RelOptSchema}
 */
 ...
 * that are not supported by the builder. If, while creating such expressions, you need to use previously built
...
private class AggCallImpl implements AggCallPlus
{
    ...
    @Override public AggregateCall aggregateCall(Registrar registrar,
        ...
        return AggregateCall.create(pos, aggFunction, distinct, approximate,
            ...
            collation, groupSet.isEmpty(), r, null, alias);
    }
