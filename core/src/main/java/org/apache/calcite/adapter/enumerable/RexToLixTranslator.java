...
public class RexToLixTranslator implements RexVisitor<RexToLixTranslator.Result> {
  ...
  /**
   * Map from RexLiteral's variable name to its literal, which is often a ({@link
   * org.apache.calcite.linq4j.tree.ConstantExpression})) It is used in the some {@code RexCall}'s
   * implementors, such as {@code ExtractImplementor}.
   *
   * @see #getLiteral
   * @see #getLiteralValue
   */
  ...
  /**
   * For {@code RexCall}, keep the list of its operand's {@code Result}. It is useful when creating
   * a {@code CallImplementor}.
   */
  private final Map<RexCall, List<Result>> callOperandResultMap = new HashMap<>();

  /**
   * Map from RexNode under specific storage type to its Result, to avoid generating duplicate code.
  ...
  /**
   * Map from RexNode to its Result, to avoid generating duplicate code. For {@code RexLiteral} and
   * {@code RexCall}.
   */
  ...
  public static Expression translateTableFunction(
      JavaTypeFactory typeFactory,
      SqlConformance conformance,
      BlockBuilder list,
      Expression root,
    ...
    final RexToLixTranslator translator =
        new RexToLixTranslator(
            ...
            conformance,
            null);
    return translator.translateTableFunction(
        rexCall, inputEnumerable, inputPhysType, outputPhysType);
  }

  /** Creates a translator for translating aggregate functions. */
  public static RexToLixTranslator forAggregation(
      JavaTypeFactory typeFactory,
      BlockBuilder list,
      @Nullable InputGetter inputGetter,
      SqlConformance conformance) {
            ...
            Expressions.lambda(
                Expressions.block(
                    Expressions.tryCatch(
                        ...
                        Expressions.catch_(e_, Expressions.return_(null, constant(null)))))),
    ...
    case BINARY:
      ...
      case VARCHAR:
        return Expressions.call(
                BuiltInMethod.STRING_TO_BINARY.method,
                operand,
    ...
    case VARCHAR:
      ...
      case DATE:
        return RexImpTable.optimize2(
            ...
            : Expressions.call(
                        Expressions.new_(BuiltInMethod.FORMAT_DATE.method.getDeclaringClass()),
                        BuiltInMethod.FORMAT_DATE.method,
                        format,
                        operand));
      ...
      case TIME:
        return RexImpTable.optimize2(
            ...
            : Expressions.call(
                        Expressions.new_(BuiltInMethod.FORMAT_TIME.method.getDeclaringClass()),
      ...
      case TIMESTAMP:
        return RexImpTable.optimize2(
            ...
            : Expressions.call(
                        Expressions.new_(BuiltInMethod.FORMAT_TIMESTAMP.method.getDeclaringClass()),
  ...
  private static Expression checkExpressionPadTruncate(
      Expression operand, RelDataType sourceType, RelDataType targetType) {
    ...
    case VARBINARY:
      ...
      default:
        if (truncate || pad) {
          ...
          return Expressions.call(method, operand, Expressions.constant(targetPrecision));
        }
        return operand;
      }
  ...
  private Expression translateCastToDate(
      RelDataType sourceType,
      Expression operand,
      ConstantExpression format,
  ...
  private Expression translateCastToTime(
      RelDataType sourceType,
      Expression operand,
      ConstantExpression format,
      Supplier<Expression> defaultExpression) {
    ...
    case VARCHAR:
      ...
      return Expressions.isConstantNull(format)
          ? Expressions.call(BuiltInMethod.STRING_TO_TIME.method, operand)
            : Expressions.call(
                Expressions.new_(BuiltInMethod.PARSE_TIME.method.getDeclaringClass()),
                BuiltInMethod.PARSE_TIME.method,
                format,
                operand);
  ...
  private Expression translateCastToTimeWithLocalTimeZone(
      RelDataType sourceType, Expression operand, Supplier<Expression> defaultExpression) {

    switch (sourceType.getSqlTypeName()) {
    case CHAR:
    case VARCHAR:
      return Expressions.call(BuiltInMethod.STRING_TO_TIME_WITH_LOCAL_TIME_ZONE.method, operand);

    case TIME:
      return Expressions.call(
            BuiltInMethod.TIME_STRING_TO_TIME_WITH_LOCAL_TIME_ZONE.method,
            RexImpTable.optimize2(
                operand, Expressions.call(BuiltInMethod.UNIX_TIME_TO_STRING.method, operand)),
              Expressions.call(BuiltInMethod.TIME_ZONE.method, root));
    ...
    case TIMESTAMP:
      return Expressions.call(
            ...
            RexImpTable.optimize2(
                operand, Expressions.call(BuiltInMethod.UNIX_TIMESTAMP_TO_STRING.method, operand)),
              Expressions.call(BuiltInMethod.TIME_ZONE.method, root));
  ...
  private Expression translateCastToTimestamp(
      RelDataType sourceType,
      Expression operand,
      ConstantExpression format,
      Supplier<Expression> defaultExpression) {
      ...
      return Expressions.isConstantNull(format)
          ...
          : Expressions.call(
              Expressions.new_(BuiltInMethod.PARSE_TIMESTAMP.method.getDeclaringClass()),
                BuiltInMethod.PARSE_TIMESTAMP.method,
                format,
                operand);
      ...
      return Expressions.add(
              Expressions.multiply(
                  Expressions.convert_(
                    Expressions.call(BuiltInMethod.CURRENT_DATE.method, root), long.class),
                  Expressions.constant(DateTimeUtils.MILLIS_PER_DAY)),
              Expressions.convert_(operand, long.class));
  ...
  private Expression translateCastToTimestampWithLocalTimeZone(
      RelDataType sourceType, Expression operand, Supplier<Expression> defaultExpression) {
    ...
    case VARCHAR:
      return Expressions.call(
            BuiltInMethod.STRING_TO_TIMESTAMP_WITH_LOCAL_TIME_ZONE.method, operand);
  ...
  /**
   * Handle checked Exceptions declared in Method. In such case, method call should be wrapped in a
   * try...catch block. " final Type method_call; try { method_call = callExpr } catch (Exception e)
  ...
  public RexNode deref(RexNode expr) {
    if (expr instanceof RexLocalRef) {
      ...
      final RexNode e2 = requireNonNull(program, "program").getExprList().get(ref.getIndex());
   ...
   *
   * @throws ControlFlowException if literal is null but {@code nullAs} is {@link
  ...
  public static Expression translateLiteral(
    ...
    case DECIMAL:
      ...
      return Expressions.new_(
            BigDecimal.class,
            Expressions.constant(requireNonNull(bd, () -> "value for " + literal).toString()));
    case DATE:
    case TIME:
    case TIME_WITH_LOCAL_TIME_ZONE:
    case INTERVAL_YEAR:
    ...
    case UUID:
      return Expressions.call(
            null,
            BuiltInMethod.UUID_FROM_STRING.method,
          Expressions.constant(literal.getValueAs(String.class)));
    case BINARY:
    case VARBINARY:
      return Expressions.new_(
            ByteString.class, Expressions.constant(literal.getValueAs(byte[].class), byte[].class));
    case GEOMETRY:
      final Geometry geom =
            requireNonNull(
                literal.getValueAs(Geometry.class),
                () -> "getValueAs(Geometries.Geom) for " + literal);
      final String wkt = SpatialTypeFunctions.ST_AsWKT(geom);
      return Expressions.call(
            null, BuiltInMethod.ST_GEOM_FROM_EWKT.method, Expressions.constant(wkt));
    case SYMBOL:
      value2 =
            requireNonNull(
                literal.getValueAs(Enum.class), () -> "getValueAs(Enum.class) for " + literal);
  ...
  /**
   * Translates the list of {@code RexNode}, using the default output types. This might be
   * suboptimal in terms of additional box-unbox when you use the translation later. If you know the
   * java class that will be used to store the results, use {@link
  ...
  /**
   * Translates the list of {@code RexNode}, while optimizing for output storage. For instance, if
   * the result of translation is going to be stored in {@code Object[]}, and the input is {@code
   * Object[]} as well, then translator will avoid casting, boxing, etc.
   *
   * @param operandList list of RexNodes to translate
   * @param storageTypes hints of the java classes that will be used to store translation results.
   *     Use null to use default storage type
   * @return translated expressions
   */
  public List<Expression> translateList(
      List<? extends RexNode> operandList, @Nullable List<? extends @Nullable Type> storageTypes) {
  ...
  private Expression translateTableFunction(
      RexCall rexCall,
      Expression inputEnumerable,
      PhysType inputPhysType,
      PhysType outputPhysType) {
  ...
  public static Expression translateCondition(
    ...
    return translator.translate(condition, RexImpTable.NullAs.FALSE);
  }
  ...
  /**
   * If an expression is a {@code NUMERIC} derived from an {@code INTERVAL}, scales it
   * appropriately; returns the operand unchanged if the conversion is not from {@code INTERVAL} to
   * {@code NUMERIC}. Does <b>not</b> scale values of type DECIMAL, these are expected to be already
   * scaled.
   */
  private static Expression scaleValue(
      RelDataType sourceType, RelDataType targetType, Expression operand) {
   ...
   * {@code RexToLixTranslator} generally produces three lines of code.
   *
   * <p>For example, when visiting a column (named commission) in table Employee, the generated code
   * snippet is:
   *
   * <blockquote>
   *
  ...
  @Override public Result visitInputRef(RexInputRef inputRef) {
    ...
    final ParameterExpression valueVariable =
        Expressions.parameter(valueExpression.getType(), list.newName("input_value"));
    ...
    final ParameterExpression isNullVariable =
        Expressions.parameter(Boolean.TYPE, list.newName("input_isNull"));
  ...
  @Override public Result visitLambdaRef(RexLambdaRef ref) {
    final ParameterExpression valueVariable =
        Expressions.parameter(typeFactory.getJavaClass(ref.getType()), ref.getName());
  ...
  @Override public Result visitNodeAndFieldIndex(RexNodeAndFieldIndex rexNodeAndFieldIndex) {
    return null;
  }
  ...
  /**
   * Visit {@code RexLiteral}. If it has never been visited before, {@code RexToLixTranslator} will
   * generate two lines of code. For example, when visiting a primitive int (10), the generated code
   * snippet is: {@code final int literal_value = 10; final boolean literal_isNull = false; }
   */
  @Override public Result visitLiteral(RexLiteral literal) {
    ...
    final Expression valueExpression =
        ...
        ? getTypedNullLiteral(literal)
            : translateLiteral(
                literal, literal.getType(), typeFactory, RexImpTable.NullAs.NOT_POSSIBLE);
    final ParameterExpression valueVariable;
    final Expression literalValue = appendConstant("literal_value", valueExpression);
    if (literalValue instanceof ParameterExpression) {
      valueVariable = (ParameterExpression) literalValue;
    } else {
      valueVariable =
          Expressions.parameter(valueExpression.getType(), list.newName("literal_value"));
      list.add(Expressions.declare(Modifier.FINAL, valueVariable, valueExpression));
    }
  ...
  /**
   * Visit {@code RexCall}. For most {@code SqlOperator}s, we can get the implementor from {@code
   * RexImpTable}. Several operators (e.g., CaseWhen) with special semantics need to be implemented
   * separately.
   */
  ...
  private static Result implementCallOperand(
      final RexNode operand,
      final @Nullable Type storageType,
      final RexToLixTranslator translator) {
  ...
  private static Expression implementCallOperand2(
      final RexNode operand,
      final @Nullable Type storageType,
      final RexToLixTranslator translator) {
    final Type originalStorageType = translator.currentStorageType;
  ...
  private Result implementPrev(RexCall call) {
    ...
    final Expression offs = Expressions.multiply(translate(offset), Expressions.constant(-1));
    requireNonNull((EnumerableMatch.PrevInputGetter) inputGetter, "inputGetter").setOffset(offs);
    return node.accept(this);
  }

  /**
   * The CASE operator is SQL’s way of handling if/then logic. Different with other {@code
   * RexCall}s, it is not safe to implement its operands first. For example: {@code select case when
   * s=0 then false else 100/s > 0 end from (values (1),(0)) ax(s); }
   */
  private Result implementCaseWhen(RexCall call) {
    final Type returnType = typeFactory.getJavaClass(call.getType());
    final ParameterExpression valueVariable =
        Expressions.parameter(returnType, list.newName("case_when_value"));
    list.add(Expressions.declare(0, valueVariable, null));
    final List<RexNode> operandList = call.getOperands();
    implementRecursively(this, operandList, valueVariable, 0);
    final Expression isNullExpression = checkNull(valueVariable);
    final ParameterExpression isNullVariable =
        Expressions.parameter(Boolean.TYPE, list.newName("case_when_isNull"));
    list.add(Expressions.declare(Modifier.FINAL, isNullVariable, isNullExpression));
    final Result result = new Result(isNullVariable, valueVariable);
    rexResultMap.put(call, result);
    return result;
  ...
  /**
   * Case statements of the form: {@code CASE WHEN a THEN b [WHEN c THEN d]* [ELSE e] END}. When
   * {@code a = true}, returns {@code b}; when {@code c = true}, returns {@code d}; else returns
   * {@code e}.
   *
   ...
   * </blockquote>
   */
  private static void implementRecursively(
      RexToLixTranslator currentTranslator,
      List<RexNode> operandList,
    ...
    if (pos == operandList.size() - 1) {
      ...
      currentBlockBuilder.add(
          Expressions.statement(
              Expressions.assign(valueVariable, EnumUtils.convert(res, valueVariable.getType()))));
      return;
    }
    ...
    final Expression tester =
        Expressions.andAlso(
            Expressions.not(testerResult.isNullVariable), testerResult.valueVariable);
    // Code for {if} branch
    final RexNode ifTrueNode = operandList.get(pos + 1);
    final BlockBuilder ifTrueBlockBuilder = new BlockBuilder(true, currentBlockBuilder);
    final RexToLixTranslator ifTrueTranslator = currentTranslator.setBlock(ifTrueBlockBuilder);
    ...
    ifTrueBlockBuilder.add(
        Expressions.statement(
            Expressions.assign(
                valueVariable, EnumUtils.convert(ifTrueRes, valueVariable.getType()))));
    ...
    final BlockBuilder ifFalseBlockBuilder = new BlockBuilder(true, currentBlockBuilder);
    final RexToLixTranslator ifFalseTranslator = currentTranslator.setBlock(ifFalseBlockBuilder);
    ...
    currentBlockBuilder.add(Expressions.ifThenElse(tester, ifTrue, ifFalse));
  }

  private Result toInnerStorageType(Result result, Type storageType) {
    final Expression valueExpression = EnumUtils.toInternal(result.valueVariable, storageType);
    if (valueExpression.equals(result.valueVariable)) {
      return result;
    }
    final ParameterExpression valueVariable =
        Expressions.parameter(
            valueExpression.getType(), list.newName(result.valueVariable.name + "_inner_type"));
    list.add(Expressions.declare(Modifier.FINAL, valueVariable, valueExpression));
  ...
  @Override public Result visitFieldAccess(RexFieldAccess fieldAccess) {
    ...
    case CORREL_VARIABLE:
      ...
      final Expression input = getter.field(list, fieldIndex, currentStorageType);
      ...
      final Expression isNullExpression =
            Expressions.condition(condition, RexImpTable.TRUE_EXPR, checkNull(valueVariable));
      list.add(Expressions.declare(Modifier.FINAL, isNullVariable, isNullExpression));
  ...
  @Override public Result visitLambda(RexLambda lambda) {
    ...
    exprTranslator.list.add(Expressions.return_(null, exprResult.valueVariable));
  ...
  Expression getLiteral(Expression literalVariable) {
    return requireNonNull(
        literalMap.get(literalVariable),
  ...
  List<Result> getCallOperandResult(RexCall call) {
    return requireNonNull(
        callOperandResultMap.get(call), () -> "callOperandResultMap.get(call) for " + call);
  }

  /**
   * Returns an expression that yields the function object whose method we are about to call.
   *
   * <p>It might be 'new MyFunction()', but it also might be a reference to a static field 'F',
   * defined by 'static final MyFunction F = new MyFunction()'.
   *
   * <p>If there is a constructor that takes a {@link FunctionContext} argument, we call that,
   * passing in the values of arguments that are literals; this allows the function to do some
   * computation at load time.
   *
   * <p>If the call is "f(1, 2 + 3, 'foo')" and "f" is implemented by method "eval(int, int,
   * String)" in "class MyFun", the expression might be "new MyFunction(FunctionContexts.of(new
   * Object[] {1, null, "foo"})".
   *
  ...
  private Expression getInstantiationExpression(Method method, RexCallBinding callBinding) {
    ...
    // use that.
    try {
      final Constructor<?> constructor = declaringClass.getConstructor(FunctionContext.class);
              ...
              constantArgs.add(
                  callBinding.isOperandLiteral(i, true)
                      ? appendConstant(
                          "_arg",
                      Expressions.constant(
                              callBinding.getOperandLiteralValue(i, Primitive.box(parameterType))))
  ...
  private Expression appendConstant(String name, Expression e) {
    if (staticList != null) {
      ...
      final String upperName = CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE, name);
